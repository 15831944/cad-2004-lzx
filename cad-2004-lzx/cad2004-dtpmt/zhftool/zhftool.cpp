...
BOOL InitSystem()
{
	...
	*/
	...
	bReadRunTime = CBlkUtility::ReadReg(HKEY_LOCAL_MACHINE, _T("SOFTWARE\\ZHFCAD"), _T("runtime"), iRumTime) ;
	if (iRumTime==0) //若第一次运行先设置菜单（cad2013）
	{
		CString strVerCAD ;
		if (CBlkUtility::ReadReg(HKEY_CURRENT_USER, _T("SOFTWARE\\Autodesk\\AutoCAD"), _T("CurVer"), strVerCAD) )
		{
			int iVerCAD = _ttoi(strVerCAD.Mid(1)) ;
			if (iVerCAD>=19)
			{
				//CString strUSOrCN ;
				//CString strKeyName ;
				//strKeyName.Format(_T("SOFTWARE\\Autodesk\\AutoCAD\%s"), strVerCAD) ;
				//CBlkUtility::ReadReg(HKEY_CURRENT_USER, strKeyName, _T("LastLaunchedLanguage"), strUSOrCN) ;
// 				CString strWorkSpaceEN[2] = {_T("Drafting & Annotation"), _T("AutoCAD Classic")} ;
// 				CString strWorkSpaceCN[2] = {_T("草图与注释"), _T("AutoCAD 经典")} ;
				// 				if (strUSOrCN==_T("zh-CN"))
				// 				{
				// 					strWorkSpace[0] = _T("草图与注释") ;
				// 					strWorkSpace[1] = _T("AutoCAD 经典") ;
				// 				}

				if (acedCommand(RTSTR, _T("menu"), RTSTR, _T("../menu/tcad.CUIX"), 0) == RTNORM)
				{
// 					if(acedCommand(RTSTR, _T("_wscurrent"), RTSTR, strWorkSpaceEN[0], 0)==RTNORM)
// 					{
// 						acedCommand(RTSTR, _T("_wscurrent"), RTSTR, strWorkSpaceEN[1], 0) ;
// 					}
// 					else
// 					{
// 						acedCommand(RTSTR, _T("_wscurrent"), RTSTR, strWorkSpaceCN[0], 0) ;
// 						acedCommand(RTSTR, _T("_wscurrent"), RTSTR, strWorkSpaceCN[1], 0) ;
// 					}
				}
				else
				{
	...
	CBlkUtility::WriteReg(HKEY_LOCAL_MACHINE, _T("SOFTWARE\\ZHFCAD"), _T("runtime"), ++iRumTime) ;
...
{
	return zhftoolDLL.ModuleResourceInstance();
}
