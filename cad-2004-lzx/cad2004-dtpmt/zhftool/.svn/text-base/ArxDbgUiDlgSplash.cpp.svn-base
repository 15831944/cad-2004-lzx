...
ArxDbgUiDlgSplash::OnInitDialog() 
{
    ...
    if (m_fh != INVALID_HANDLE_VALUE) {
        ...
        GetDlgItem(IDC_STATIC_SPLASHFRAME)->SetWindowPos(NULL, kBorderX, kBorderY, m_picWidth, m_picHeight, SWP_NOZORDER);
        displayTextLines();
        CenterWindow();
    }
...
ArxDbgUiDlgSplash::OnPaint() 
{
    ...
    if (m_fh != INVALID_HANDLE_VALUE && plgPalette) {
        ...
        if (pPixels) {
            ...
            CRect rect;
            GetDlgItem(IDC_STATIC_SPLASHFRAME)->GetClientRect(&rect);
            GetDlgItem(IDC_STATIC_SPLASHFRAME)->ClientToScreen(&rect);
            ScreenToClient(&rect);
...
{
    int height;
    CWnd* wnd = GetDlgItem(IDC_STATIC_TXTLINE1);
    ASSERT(wnd != NULL);
    if (wnd) {
        CRect linerect;
    ...
    int numLines = 0;
    processTextLine(m_str1, IDC_STATIC_TXTLINE1, numLines, height);
    processTextLine(m_str2, IDC_STATIC_TXTLINE2, numLines, height);
    processTextLine(m_str3, IDC_STATIC_TXTLINE3, numLines, height);
    processTextLine(m_str4, IDC_STATIC_TXTLINE4, numLines, height);
