...
// 			}
		...
		else if (iInput==9) //将信号机图块基点的小竖线长度变为0.01
		{
			AcDbBlockTable * pBlkTbl = NULL ;
...
// 			pBlkTblRecIter = NULL ;
		...
		else if (iInput==12)
		{
			...
			CBlkUtility::FindFile(_T("D:\\逻辑检查\\定型"), _T("dwg"), strArrayFilePath) ;
			int iCount = strArrayFilePath.GetCount() ;
			acutPrintf(_T("\n%d"), iCount) ;
			long nMaxId = 0 ;
			for (int i=0; i<iCount; i++)
			{
				...
				AcDbDatabase* pCurDb = new AcDbDatabase(false) ;
				mysys.m_pDb = pCurDb ;
				if (Acad::eOk== pCurDb->readDwgFile(strFilePath))
				{
					long nId = mysys.GetMaxId() ;
					acutPrintf(_T("\n%d:cur maxid %d,%s"),i, nId, strFilePath) ;
					if (nId>nMaxId)
					{
						nMaxId = nId ;
					}
		...
		{
// 			TiXmlDocument doc(_T("d:\\test\\demotest.xml")) ;
// 			bool bLoadOk = doc.LoadFile() ;
// 			if (bLoadOk)
// 			{
// 				TiXmlPrinter tiPrinter ;
// 				doc.Accept(&tiPrinter) ;
// 				acutPrintf(_T("\n%s"), tiPrinter.CStr()) ;
// 			}
		}
	}
			...
			if(Acad::eOk == acdbOpenAcDbEntity(pEnt,ent_id, AcDb::kForRead))
			{
				...
				if (iNumVert>1)
				{
					...
					else
					{
						...
						if (iNumVert==3) //若3个顶点时，此时pt_2d_nearstart和pt_2d_nearend为同一个点，即序号为1的点，故上面代码若变动了pt_2d_nearstart，则应该同时更新pt_2d_nearend
						{
							pt_2d_nearend = pt_2d_nearstart ;
						}
						//终点
...
void CUserTools::BatchDoDwg2(void* pData)
{
	CToCadExecuteData *pToData = (CToCadExecuteData *)pData ;
	//CString strFile ;
	//strFile.Format(_T("%s"), (TCHAR*)pData) ;

	TCHAR * chFileName = NULL ;
	chFileName = (LPTSTR)(LPCTSTR)(pToData->strFileName);
	TCHAR * chCommand = NULL ;
	chCommand = (LPTSTR)(LPCTSTR)(pToData->strCommand);


	AcApDocument* pDoc = acDocManager->curDocument();
	if (acDocManager->isApplicationContext()) 
	{
		acDocManager->appContextOpenDocument(chFileName);
		...
		for (;!pIterDoc->done(); pIterDoc->step())
		{
			AcApDocument *pDoc = pIterDoc->document() ;
			CString strDoc= pDoc->fileName() ;
			if (strDoc.Compare(pToData->strFileName)==0)
			{
				//acDocManager->sendStringToExecute(pDoc, _T("-purge a \r n \r \003\003")) ;
				//begin 刷新居中保存
				//acDocManager->sendStringToExecute(pDoc, _T("zoom e \r \003\003")) ;
				acDocManager->sendStringToExecute(pDoc, chCommand) ;
				acDocManager->sendStringToExecute(pDoc, _T("\003\003")) ;
...
// 				//end 单位
				...
				//begin 设为电路图
// 				acDocManager->sendStringToExecute(pDoc, _T("savecircuit  \r \003\003")) ;
// 				acDocManager->sendStringToExecute(pDoc, _T("_qsave  \r \003\003")) ;	
// 				acDocManager->sendStringToExecute(pDoc, _T("close \n")) ;	
				//end 设为电路图
	...
	if (iType==0) //信号机
	{
		...
		if (strBlkName.Left(3)==_T("XHJ"))
		{
			...
			if (acedGetPoint(NULL, _T("\n选择插入基点"), pt_insert)==RTNORM )
			{
				...
				int iXhjWithJyj = 1 ;
				CBlkUtility::QueryValueReg(_T("PMTRBSET"), _T("XhjWithJyj"), iXhjWithJyj) ;
				if (iXhjWithJyj==1)
				{
	...
	else if (iType==1) //道岔
	{
		...
		if (acedGetPoint(NULL, _T("\n选择插入基点"), pt_insert)==RTNORM )
		{
			...
			if (acedGetPoint(NULL, _T("\n指定一个道岔朝向点"), pt_direct)==RTNORM )
			{
				...
				mysys.ConvertPtAdsTo3D(pt_insert, pt_3d_insert) ;
				strArrayVal.Add(strName) ;	
				if (mysys.InsertBlock(pRef, _T("DC_000"), pt_3d_insert, &strArrayVal, FALSE))
				{				
					//道岔朝向
...
int CUserTools::SetBlkAtt()
{
	...
	if (dlg.DoModal()==IDOK)
	{
		if (dlg.m_iRange==0)//全图
		{
			CLongArray nArrayHandle ;
			iRet = mysys.FindBlk(dlg.m_strBlkName, nArrayHandle) ;
			for (int i=0; i<nArrayHandle.GetCount(); i++)
			{
				mysys.SetAttValue(nArrayHandle.GetAt(i), dlg.m_grid.m_vecStringCell) ;

				if (dlg.m_iColorIndex!=7)
				{
					for (int j=0; j<dlg.m_grid.m_vecStringCell.size(); j++)
					{
						mysys.SetAttColor(nArrayHandle.GetAt(i), dlg.m_grid.m_vecStringCell[j][0], dlg.m_iColorIndex+1) ;
		...
		else //选择
		{
			ads_name temp_name ;
			long lLength = 0 ;
			ads_sslength(dlg.m_ss, &lLength) ;
			iRet = lLength ;
			for(int i = 0; i<lLength; i++)
			{
				ads_ssname(dlg.m_ss, i, temp_name) ;
				AcDbEntity * pEnt = NULL ;
				if (mysys.OpenAcDbEntity(temp_name, pEnt))
				{
					if (pEnt->isKindOf(AcDbBlockReference::desc()))
					{
						...
						mysys.SetAttValue(pRef, dlg.m_grid.m_vecStringCell) ;	
						if (dlg.m_iColorIndex!=7)
						{
							for (int j=0; j<dlg.m_grid.m_vecStringCell.size(); j++)
							{
								mysys.SetAttColor(pRef, dlg.m_grid.m_vecStringCell[j][0], dlg.m_iColorIndex+1) ;
...
//批量对指定文件夹下的图纸进行指定的操作（发送指定命令）
void CUserTools::BatchDoDwg(const CString& strCommand)
{
	CSBDestination sb(NULL, IDS_BFF_TITLE) ;
	sb.SetFlags(BIF_RETURNONLYFSDIRS|BIF_STATUSTEXT) ;
	if (sb.SelectFolder())
	{
		...
		for (int i=0; i<strArrayFile.GetCount(); i++)
		{
			CString strFile = strArrayFile.GetAt(i) ;
			AcApDocManager *pDocMgr = acDocManager ;
			CToCadExecuteData toData ;
			toData.strFileName = strFile ;
			toData.strCommand = strCommand ;
			//TCHAR * chFileName = NULL ;
			//chFileName = (LPTSTR)(LPCTSTR)strFile ;
			pDocMgr->executeInApplicationContext(BatchDoDwg2,(void *)&toData) ;
		}
	}
}
...
void CUserTools::ShowBlkAtt(void)
{
	...
	if (dlg.DoModal()==IDOK)
	{
		...
		if (dlg.m_iRange==1)//全图
		{
			CLongArray nArrayHandle ;
			iRet = mysys.FindBlk(dlg.m_strBlkName, nArrayHandle) ;
			for (int i=0; i<nArrayHandle.GetCount(); i++)
			{
				mysys.ShowAtt(nArrayHandle.GetAt(i), dlg.m_strAtt, dlg.m_iShowOrHide) ;
			}										
		}
		else //选择
		{
			...
			for(int i=0; i<lLength; i++)
			{
				...
				if (mysys.OpenAcDbEntity(temp_name, pEnt))
				{
					...
					{
						AcDbBlockReference * pRef = AcDbBlockReference::cast(pEnt) ;
						mysys.ShowAtt(pRef, dlg.m_strAtt, dlg.m_iShowOrHide) ;
					}
					pEnt->close() ;				
				}							
			}					
...
int CUserTools::JoinLine(void)
{
	...
	if (bHasErro==TRUE)
	{
		strMsg.Format(_T("合并%d处多段线,已标示。"), iNumJoin) ;
		AfxMessageBox(strMsg) ;		
	}
	return 0;
}
...
void CUserTools::SetPlineWidth(ads_name ss, double dFrom, double dTo, BOOL bIncludeBlk)
{
	CUserSystem mysys ;
	long nLen = 0 ;
	...
	for(int i=0; i<nLen; i++)
	{
		AcDbEntity *pEnt = NULL ;
		ads_name temp_name ;
		acedSSName(ss, i, temp_name) ;
		if(mysys.OpenAcDbEntity(temp_name, pEnt, AcDb::kForRead))
		{
			if (pEnt->isKindOf(AcDbPolyline::desc()))
			{
				pEnt->upgradeOpen() ;
...
void CUserTools::SetPlineWidth(void)
{
	...
	{
		ads_name ss ;
		if (dlg.m_iComoBoxIndex==0)
		{
			if (acedSSGet(_T("A"), NULL, NULL, NULL, ss)==RTNORM)
			{
				CUserTools::SetPlineWidth(ss, dlg.m_dFrom, dlg.m_dTo, dlg.m_bIncludeBlk) ;
			}
		}
		else
		{
			CUserTools::SetPlineWidth(dlg.m_ss, dlg.m_dFrom, dlg.m_dTo, dlg.m_bIncludeBlk) ;
		}
	}
}
