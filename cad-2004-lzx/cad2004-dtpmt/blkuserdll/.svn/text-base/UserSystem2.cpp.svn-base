...
BOOL CUserSystem::SetAttPosition(AcDbBlockReference * pRef , CString strTag, double x_dist,double y_dist)
{
	BOOL bReturn = FALSE ;
	AcDbAttribute * pAtt = NULL ;
	if( this->FindAtt ( pRef , strTag , pAtt ) )
	{
		...
		AcGePoint3d pt_3d_position=pAtt->position();
		pt_3d_position.x+=x_dist;
		pt_3d_position.y+=y_dist;
		double Angle=pRef->rotation();
		if(Angle==PI)
		{
			Angle=Angle;
			pt_3d_position.x=0;
...
BOOL CUserSystem::SetAttTextAlignMode(AcDbBlockReference * pRef, CString strTag, AcDb::TextHorzMode textHMode,AcDb::TextVertMode textVMode)
{
	BOOL bReturn = FALSE ;
	AcDbAttribute * pAtt = NULL ;
	//CString strValue ;
	...
	if( this->FindAtt ( pRef , strTag , pAtt ) )
	{
		pAtt->upgradeOpen() ;
		//pAtt->setTextString ( strValue ) ;
		//AcDb::TextHorzMode textHMode=AcDb::kTextLeft, AcDb::TextVertMode textVMode=AcDb::kTextBase
	...
	{
		this->AddAttValue(pRef, strTag, _T(""), pRef->position(), 0, FALSE) ;
		if( this->FindAtt ( pRef , strTag , pAtt ) )
		{
			pAtt->upgradeOpen() ;
			//pAtt->setTextString ( strValue ) ;
...
BOOL CUserSystem::SetAttColor(long nHandle, CString strTag, Adesk::UInt16 nColorIndex)
{
	BOOL bReturn = FALSE ;
	AcDbEntity* pEnt = NULL ;
	if (this->OpenAcDbEntity(nHandle, pEnt, AcDb::kForRead))
	{
		...
		{
			AcDbBlockReference* pRef = AcDbBlockReference::cast(pEnt) ;
			bReturn = SetAttColor(pRef, strTag, nColorIndex) ;
		}
		pEnt->close() ;
	}
	return bReturn ;
...
BOOL CUserSystem::GetBlockName(ads_name en, CString &strBlockName) 
{
	BOOL bRet = FALSE ;

	AcDbEntity* pEnt = NULL ;
	if (this->OpenAcDbEntity(en, pEnt, AcDb::kForRead))
	{
		if (pEnt->isKindOf(AcDbBlockReference::desc()))
		{
			AcDbBlockReference* pRef = AcDbBlockReference::cast(pEnt) ;
