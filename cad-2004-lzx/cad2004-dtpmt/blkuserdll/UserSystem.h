...
public:
	...
	BOOL GetBlockName(ads_name en, CString &strBlockName) ;
...
// 	int FindReplaceBlock(CString strOldBlockName, CString strNewBlockFile, int nByBlockName, BOOL bIsGlobal=TRUE) ;
	...
	//取得块的属性tag及值
	int GetAttValue(AcDbBlockReference* pRef, vector<vector<CString > > &vec);
	int GetAttValue(long nHandle, vector<vector<CString > > &vec);
	//获取指定图块 指定属性的值；
	...
	//设置块的属性tag及值
	int SetAttValue(AcDbBlockReference* pRef, vector<vector<CString > > &vec);
	int SetAttValue(long nHandle, vector<vector<CString > > &vec);
	...
	BOOL SetAttColor(long nHandle, CString strTag, Adesk::UInt16 nColorIndex=7) ;
	...
	BOOL ShowAtt(long nHandle, const TCHAR * chTag, bool bShow);
	...
	//获取实体颜色
	void GetEntColorIndex(AcDbObjectId objId, Adesk::UInt16 &iColorIndex);
	void GetEntColorIndex(ads_name en, Adesk::UInt16 &iColorIndex);
	void GetEntColorIndex(long nHandle, Adesk::UInt16 & iColorIndex);
	...
	// 高亮实体
	void Highlight(ads_name en);
	// 取消高亮实体
	void UnHighlight(long nHandle);
	// 取消高亮实体
	void UnHighlight(ads_name en);
	...
	int FindEnt(const CString& strAppName, const CString strXdata, CLongArray& nArrayHandle);
	...
	void TransMySnToId(void);
	BOOL SetEntConnect(AcDbEntity *pEnt, long nSecondId , long nConCableId) ;

	// 绘制标准图框（包括边线，签字栏等）
	void DrawFrame(const AcGePoint3d& pt_3d_leftbuttom, double dWidth, CStringArray& strArrayVal, const CString& strSignTemplete, CString strFileName=_T("XXX图纸"), int iSignHorOrVer=0);
	...
	int GetTextAlignMode(const AcDbText* pText);
	// 取得对于文字翻转后，其对齐方式iAlignModeOld应该变为值
	int GetTextAlignMode(int iAlignModeOld);
	int GetPLineStartWidth(long nHandle, double &dWidth);
	BOOL SetAttTextAlignMode(AcDbBlockReference * pRef, CString strTag, AcDb::TextHorzMode textHMode=AcDb::kTextLeft, AcDb::TextVertMode textVMode=AcDb::kTextBase);
	BOOL SetAttPosition(AcDbBlockReference * pRef , CString strTag, double x_dist=0,double y_dist=0);
//	BOOL SetAttTextStyle(AcDbBlockReference * pRef, CString strTag, AcDb::TextHorzMode textHMode,AcDb::TextVertMode textVMode);
	//BOOL ExcelToDwg(CString strExcelFileName, int iStartRow, int iEndRow, TCHAR iStartCol[], TCHAR iEndCol[], ads_point base_pt, double dHeightRate=0.328, double dWidthRate=2.41);
};
