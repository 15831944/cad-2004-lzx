...
// 显示或隐藏块指定的属性
BOOL CUserSystem::ShowAtt(long nHandle, const TCHAR * chTag, bool bShow)
{
	BOOL bRet = FALSE ;
	AcDbEntity* pEnt = NULL ;
	if (this->OpenAcDbEntity(nHandle, pEnt))
	{
		if (pEnt->isKindOf(AcDbBlockReference::desc()))
		{
			...
			bRet = this->ShowAtt(pRef, chTag, bShow) ;
		}
		pEnt->close() ;
	}
		...
		for(pAttItor->start(); !pAttItor->done(); pAttItor->step())
		{
			...
			if (OpenAcDbEntity(objId, pEntAtt, AcDb::kForWrite))
			{
				...
				if (dAngleAtt>(PI/2)&&dAngleAtt<(PI*3/2))
				{
					...
					int iAlignMode = this->GetTextAlignMode(pAtt) ;
					int iAlignModeNew = this->GetTextAlignMode(iAlignMode) ;
					if (iAlignModeNew!=-1)
					{
						this->ConvertTextMode((AcDbText*&)pAtt, iAlignModeNew) ;
					}
				}
...
Acad::ErrorStatus CUserSystem::MirrEnt(AcDbEntity* pEnt, const AcGePoint3d& pt_3d_1, const AcGePoint3d& pt_3d_2)
{
	...
	{
		AcDbBlockReference* pRef = AcDbBlockReference::cast(pEnt) ;
		AcGeScale3d scale_3d_old = pRef->scaleFactors() ;
		if (scale_3d_old.sy<0)
		{
			BOOL bWriteable = pRef->isWriteEnabled() ;
			if (!bWriteable)
			{
				pRef->upgradeOpen() ;
			}
			scale_3d_old.set(-scale_3d_old.sx, -scale_3d_old.sy, scale_3d_old.sz) ;
			pRef->setScaleFactors(scale_3d_old) ;
		...
		dAngleOld = pRef->rotation() ;
		dAngleOld = (scale_3d_old.sx<0? fmod(dAngleOld+PI,PI*2):dAngleOld) ;
		//acutPrintf(_T("\nBlock old rotation:%.3f"), dAngleOld*180/PI) ;
	}
...
this->SetSysVar(_T("MIRRTEXT"), iVarOld) ;
		if(pEnt->isKindOf(AcDbBlockReference::desc()))
		{
			...
			for(pAttItor->start(); !pAttItor->done(); pAttItor->step())
			{
				...
				{
					AcDbAttribute *pAtt = AcDbAttribute::cast(pEntAtt) ;					
					acutPrintf(_T("\nBlock new rotation:%.3f"), dAngle*180/PI) ;
					dAngle = fmod(dAngle+(scale_3d.sx<0?PI:0)+dAngleOld,PI*2); //scale=-1时相当于旋转了180度，后面加上原块（旋转前而不是初始图块，可能已经被镜像旋转多次）的角度
					//acutPrintf(_T("\nfmod(dAngle+(scale_3d.sx<0?PI:0)+dAngleOld,PI*2):%.3f"), dAngle*180/PI) ;
...
{
	AcDbObjectId objId ;
	if(GetObjectId(objId, nHandle)==eOk)
	{
		return SetEntColorIndex(objId, iColorIndex) ;
	}
	else
...
//获取实体颜色
void CUserSystem::GetEntColorIndex(AcDbObjectId objId, Adesk::UInt16 &iColorIndex)
{
	Acad::ErrorStatus es ;
	AcDbEntity *pEnt = NULL ;
	if(this->OpenAcDbEntity(objId, pEnt, AcDb::kForRead))
	{			
		//es = pEnt->setColorIndex(iColorIndex) ;
		iColorIndex=pEnt->colorIndex();
		pEnt->close() ;
	}
	//return es ;
}
void  CUserSystem::GetEntColorIndex(ads_name en, Adesk::UInt16 &iColorIndex)
{
	AcDbObjectId objId ;
	if(acdbGetObjectId(objId, en)==eOk)
	{
		return GetEntColorIndex(objId, iColorIndex) ;
	}
}
void  CUserSystem::GetEntColorIndex(long nHandle, Adesk::UInt16 & iColorIndex)
{
	AcDbObjectId objId ;
	if(GetObjectId(objId, nHandle)==eOk)
	{
...
// 高亮实体
void CUserSystem::Highlight(ads_name en)
{
	long nLen = 0 ;
	acedSSLength(en, &nLen) ;
	for(int i=0; i<nLen; i++)
	{
		AcDbEntity *pEnt = NULL ;
		ads_name temp_name ;
		acedSSName(en, i, temp_name) ;
		if(this->OpenAcDbEntity(temp_name, pEnt, AcDb::kForRead))
		{
			pEnt->highlight();
			::acedUpdateDisplay();
			pEnt->close() ;
...
// 取消高亮实体
void CUserSystem::UnHighlight(ads_name en)
{
	long nLen = 0 ;
	acedSSLength(en, &nLen) ;
	for(int i=0; i<nLen; i++)
...
int CUserSystem::FindEnt(const CString& strAppName, const long nXdata, CLongArray& nArrayHandle)
{
	...
	for(pBlkTblRecIter->start();!pBlkTblRecIter->done();pBlkTblRecIter->step())
	{
		...
		{
			long nXdataTmp = 0 ;
			if (this->GetXdata(pEnt, strAppName, nXdataTmp) )
			{
				if (nXdataTmp==nXdata)
				{
...
// 查找具有指定app扩展数据的实体
int CUserSystem::FindEnt(const CString& strAppName, const CString strXdata, CLongArray& nArrayHandle)
{
	int iRet = 0 ;
	...
	for(pBlkTblRecIter->start();!pBlkTblRecIter->done();pBlkTblRecIter->step())
	{
		...
		{
			CString strXdataTmp  ;
			if (this->GetXdata(pEnt, strAppName, strXdataTmp) )
			{
				if (strXdataTmp==strXdata)
				{
					long nHandle = this->GetHandle(pEnt) ;
					nArrayHandle.Add(nHandle) ;
					iRet++ ;
...
void CUserSystem::TransMySnToId(void)
{
	AcDbBlockTable * pBlkTbl = NULL ;
	m_pDb->getBlockTable(pBlkTbl,AcDb::kForRead);
	AcDbBlockTableRecord * pBlkTblRec = NULL ;
	...
	for(pBlkTblRecIter->start();!pBlkTblRecIter->done();pBlkTblRecIter->step())
	{
		...
		if (Acad::eOk == pBlkTblRecIter->getEntity(pEnt_1, AcDb::kForWrite))
		{
			if (pEnt_1->isKindOf(AcDbText::desc()))
			{
				...
				int iHasTrans = 0 ;
				if (this->GetXdata(pEnt_1, _T("HASTRANS"), iHasTrans)==FALSE)
				{
					//begin 处理first_sn
					CString strFirstSn ;
					if (this->GetXdata(pEnt_1, _T("FIRST_SN"), strFirstSn))
					{
						//查看firsts_sn是否已经有对应的con_1_id,有则不用处理了
						int iCon_1_Id = 0 ;
						if (!this->GetXdata(pEnt_1, _T("CON_1_ID"), iCon_1_Id))
						{
							//先找出对方端子
							CLongArray nArraySecondHandle ;
							int iSecHandleSum = this->FindEnt(_T("MY_SN"), strFirstSn, nArraySecondHandle) ;
							if (iSecHandleSum>0)
							{
								//先看本端子是否有关联电缆类型
								CString strFirstConSn = _T("") ;							
								CLongArray nArrayConTypeHandle ;
								if(this->GetXdata(pEnt_1, _T("FIRST_CON_SN"), strFirstConSn))
								{
									int iConTypeHandleSum = this->FindEnt(_T("MY_SN"), strFirstConSn, nArrayConTypeHandle) ;
								...
								if(this->OpenAcDbEntity(nArraySecondHandle.GetAt(0), pEnt_2))
								{
									//若在第一个端子xdata中没有连接电缆类型sn，试着去第二个端子找找
									if (nArrayConTypeHandle.GetCount()==0)
									{
										if(this->GetXdata(pEnt_2, _T("FIRST_CON_SN"), strFirstConSn))
										{
											int iConTypeHandleSum2 = this->FindEnt(_T("MY_SN"), strFirstConSn, nArrayConTypeHandle) ;
									...
									if (nArrayConTypeHandle.GetCount()!=0)
									{
										this->OpenAcDbEntity(nArrayConTypeHandle.GetAt(0), pEnt_3) ;
									}

									//添加ID
									long nFirst_Id = 0 , nSecond_Id = 0 ;
									//本端子
									if(!this->GetXdata(pEnt_1 , _T("ID") , nFirst_Id))
									{
										nFirst_Id = nMaxId+1 ;
										nMaxId++ ;
										this->SetXdata(pEnt_1 , _T("ID") , nFirst_Id) ;
									}
									//连接到的端子
									if(!this->GetXdata(pEnt_2 , _T("ID") , nSecond_Id))
									{
										nSecond_Id = nMaxId+1 ;
										nMaxId++ ;
										this->SetXdata(pEnt_2 , _T("ID") , nSecond_Id) ;
									}
									//连接电缆类型（若有）
									long nCon_Cable_Id = 0 ;
									if(pEnt_3 != NULL)
									{
										if(!this->GetXdata(pEnt_3 , _T("ID") , nCon_Cable_Id))
										{
											nCon_Cable_Id = nMaxId+1 ;
											nMaxId++ ;
											this->SetXdata(pEnt_3 , _T("ID") , nCon_Cable_Id) ;
										}
									}

									//关联端子
									if(this->SetEntConnect(pEnt_1, nSecond_Id, nCon_Cable_Id))
									{
										this->SetEntConnect(pEnt_2, nFirst_Id, nCon_Cable_Id) ;
									}
					...
					//begin 处理second_sn 
					CString strSecondSn ;
					if (this->GetXdata(pEnt_1, _T("SECOND_SN"), strSecondSn))
					{
						//查看second_sn是否已经有对应的con_2_id,有则不用处理了
						int iCon_2_Id = 0 ;
						if (!this->GetXdata(pEnt_1, _T("CON_2_ID"), iCon_2_Id))
						{
							//先找出对方端子
							CLongArray nArrayOtherHandle  ;
							int iOtherHandleSum = this->FindEnt(_T("MY_SN"), strSecondSn, nArrayOtherHandle) ;
							if (iOtherHandleSum>0)
							{
								//先看本端子是否有关联电缆类型
								CString strSecondConSn = _T("") ;
								CLongArray nArrayConTypeHandle  ;								
								if(this->GetXdata(pEnt_1, _T("SECOND_CON_SN"), strSecondConSn))
								{
									int iConTypeHandleSum = this->FindEnt(_T("MY_SN"), strSecondConSn, nArrayConTypeHandle) ;
									if (iConTypeHandleSum>1)
									{
								...
								AcDbEntity *pEnt_2 = NULL ; //连接的端子
								AcDbEntity *pEnt_3 = NULL ; //连接的电缆类型
								if(this->OpenAcDbEntity(nArrayOtherHandle.GetAt(0), pEnt_2))
								{
									//若在第一个端子xdata中没有连接电缆类型sn，试着去第二个端子找找(此处没意义)
									if (nArrayConTypeHandle.GetCount()==0)
									{
										if(this->GetXdata(pEnt_2, _T("SECOND_CON_SN"), strSecondConSn))
										{
											int iConTypeHandleSum2 = this->FindEnt(_T("MY_SN"), strSecondConSn, nArrayConTypeHandle) ;
											if (iConTypeHandleSum2>1)
											{
												acutPrintf(_T("\n!!!!!!!!!!AppName为 MY_SN，值为%s的记录不止一条，请注意！"), strSecondConSn) ;
											}
					...
					if (this->GetXdata(pEnt_1, _T("PAR_SN"), strFirstConSn)) 
					{
						long iParID = 0 ; 
						if (!this->GetXdata(pEnt_1, _T("PARENT_ID"), iParID))
						{
							//先找出对方端子
							CLongArray nArrayParHandle  ;
							int iParHandleSum = this->FindEnt(_T("MY_SN"), strFirstConSn, nArrayParHandle) ;
							if (iParHandleSum>0)
							{
								AcDbEntity *pEntPar = NULL ; //位置								
								if(this->OpenAcDbEntity(nArrayParHandle.GetAt(0), pEntPar))
								{
									long iID = 0 ;
									//父端子已有ID
									if (this->GetXdata(pEntPar, _T("ID"), iID))
									{
										this->SetXdata(pEnt_1, _T("PARENT_ID"), iID) ;
									}
									else
									{
										iParID = nMaxId+1 ;
										nMaxId++ ;
										this->SetXdata(pEntPar, _T("ID"), iParID) ;
										this->SetXdata(pEntPar, _T("IS_PARENT"), 1) ;
										this->SetXdata(pEnt_1, _T("PARENT_ID"), iParID) ;
									}
					...
					this->SetXdata(pEnt_1, _T("HASTRANS"), 1) ;
				}
			}
...
BOOL CUserSystem::SetEntConnect(AcDbEntity *pEnt, long nSecondId , long nConCableId)
{
	BOOL bReturn = FALSE ;
	//读取连接;
	long nConId = 0 ;
	...
	if(!this->GetXdata(pEnt, _T("CON_1_ID") , nConId))
	{
		this->SetXdata(pEnt , _T("CON_1_ID") , nSecondId) ;
		if(nConCableId > 0)
		{
			this->SetXdata (pEnt , _T("CON_1_CABLE_ID") , nConCableId) ;
		}
		pEnt->setColor(color) ;
		bReturn = TRUE ;
	}
	...
	{
		this->SetXdata(pEnt , _T("CON_2_ID") , nSecondId) ;
		if(nConCableId > 0)
		{
			this->SetXdata (pEnt , _T("CON_2_CABLE_ID") , nConCableId) ;
		}
		pEnt->setColor(color) ;
		bReturn = TRUE ;
	}
...
// 绘制标准图框（包括边线，签字栏等）
void CUserSystem::DrawFrame(const AcGePoint3d& pt_3d_leftbuttom, double dWidth, CStringArray& strArrayVal, const CString& strSignTemplete, CString strFileName, int iSignHorOrVer)
{
	CString strTempletFileName = _T("frame_left") ;
	AcDbBlockReference* pRef = NULL ;
	CStringArray strArrayValLeft ;
	strArrayValLeft.Add(strFileName) ;
	if (this->InsertBlock(pRef, strTempletFileName, pt_3d_leftbuttom, &strArrayValLeft, FALSE))
	{
		pRef->close() ;
		pRef = NULL ;
	}

	AcGePoint3d pt_3d_frame[2] ;
	AcGePoint3dArray pt3dArray1, pt3dArray2 ;
	//frist point(LB)
	pt_3d_frame[0].x = pt_3d_leftbuttom.x+25 ;
	pt_3d_frame[0].y = pt_3d_leftbuttom.y ;
	pt3dArray1.append(pt_3d_frame[0]) ;
	pt_3d_frame[1].x = pt_3d_frame[0].x ;
	pt_3d_frame[1].y = pt_3d_frame[0].y+5 ;
	pt3dArray2.append(pt_3d_frame[1]) ;

	//second point(RB)
	pt_3d_frame[0].x = pt_3d_leftbuttom.x+25+dWidth+5 ;
	pt3dArray1.append(pt_3d_frame[0]) ;
	pt_3d_frame[1].x = pt_3d_frame[0].x-5 ;
	pt3dArray2.append(pt_3d_frame[1]) ;

	AcGePoint3d pt_3d_sign ;
	pt_3d_sign = pt_3d_frame[1] ; //签字栏基点
	...
	this->AddAcDbPolyLine(pt3dArray1,0) ; //外边框线
	this->AddAcDbPolyLine(pt3dArray2, 0.5) ;//内边框线

	//签字栏
	if (this->InsertBlock(pRef, strSignTemplete, pt_3d_sign, &strArrayVal, FALSE))
	{
		if (iSignHorOrVer==1) //签字栏竖直放
		{
			double dWidthSign = 180 ;
			if (strSignTemplete.Right(1)==_T("6"))
			{
				dWidthSign = 210 ;
			}
			AcGeMatrix3d matrix ;
			AcGeVector3d vec(0, dWidthSign, 0) ;
			matrix.setToTranslation(vec) ;
			pRef->transformBy(matrix) ; 
			this->RotationEnt(pRef,PI/2, pRef->position() ) ;
			//pRef->setRotation(PI/2) ;
		}
		pRef->close() ;
		pRef = NULL ;
	}
...
//取得块的属性tag及值
int CUserSystem::GetAttValue(AcDbBlockReference* pRef, vector<vector<CString > > &vec)
{
	int iRet = 0;
	...
	{
		objAttId = pObjIter->objectId();
		Acad::ErrorStatus es = pRef->openAttribute(pAtt, objAttId, AcDb::kForRead, Adesk::kFalse);
		CString strTag, strText ;
		strTag.Format(_T("%s"), pAtt->tag()) ;
		strText.Format(_T("%s"),  pAtt->textString()) ;
		vector<CString> vecTmp ;
		vecTmp.push_back(strTag) ;
		vecTmp.push_back(strText) ;
...
//设置块的属性tag及值
int CUserSystem::SetAttValue(AcDbBlockReference* pRef, vector<vector<CString > > &vec)
{
	int iRet = 0;
	for (int i=0; i<vec.size(); i++)
	{
		...
		strTag = vec[i][0] ;
		strText = vec[i][1] ;
		this->SetAttValue(pRef, strTag, strText) ;
	}
...
//取得块的属性tag及值
int CUserSystem::GetAttValue(long nHandle, vector<vector<CString > > &vec)
{
	int iRet = 0;
	AcDbEntity* pEnt = NULL ;
	if (this->OpenAcDbEntity(nHandle, pEnt, AcDb::kForRead))
...
//设置块的属性tag及值
int CUserSystem::SetAttValue(long nHandle, vector<vector<CString > > &vec)
{
	int iRet = 0;
...
//0(Left)/1(Center)/2(Right)/3(Aligned)4(Middle默认)/5(Fit)/6(TopLeft)/7(TopCenter)/8(TopRight)/9(MiddleLeft)/10(MiddleCenter)/11(MiddleRight)/12(BottomLeft)/13(BottomCenter)/14(BottomRight
int CUserSystem::GetTextAlignMode(const AcDbText* pText)
{
	int iTextMode = -1 ;

	AcDb::TextVertMode iVerMode = pText->verticalMode() ;
	AcDb::TextHorzMode iHorMode = pText->horizontalMode() ;
	this->ConvertTextModeToIndex(iHorMode, iVerMode, iTextMode) ;

	return iTextMode;
}

// 取得对于文字翻转后，其对齐方式iAlignModeOld应该变为值
int CUserSystem::GetTextAlignMode(int iAlignModeOld)
{
	int iAlignModeNew = -1 ;
	switch (iAlignModeOld)
	{
	case kDTextLeft:
		iAlignModeNew = kDTextRight ;
		break;
	case kDTextRight:
		iAlignModeNew = kDTextLeft ;
		break;
	case kDTextTopLeft:
		iAlignModeNew = kDTextBottomRight ;
		break;
	case kDTextTopRight:
		iAlignModeNew = kDTextBottomLeft ;
		break;
	case kDTextMiddleLeft:
		iAlignModeNew = kDTextMiddleRight ;
		break;
	case kDTextMiddleRight:
		iAlignModeNew = kDTextMiddleLeft ;
		break;
	case kDTextBottomLeft:
		iAlignModeNew = kDTextTopRight ;
		break;
	case kDTextBottomRight:
		iAlignModeNew = kDTextTopLeft;
		break;
	default:
		break;
	}

	return iAlignModeNew;
}
