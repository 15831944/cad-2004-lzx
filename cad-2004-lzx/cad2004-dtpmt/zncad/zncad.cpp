...
//-----------------------------------------------------------------------------
//----- zncad.cpp : Initialization functions
//-----------------------------------------------------------------------------
#include "StdAfx.h"
//#include "resource.h"
#include <afxdllx.h>

BOOL bFirst = TRUE ; //是否是第一次初始ole环境 ;
...
AC_IMPLEMENT_EXTENSION_MODULE(zncadDLL)
...
#ifdef NEVER
AFX_EXTENSION_MODULE zncadExtDLL ={ NULL, NULL } ;
#endif
...
BOOL WINAPI DllMain (HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved) {
	...
	if ( dwReason == DLL_PROCESS_ATTACH ) {
		...
		zncadDLL.AttachInstance (hInstance) ;
		InitAcUiDLL () ;
	} else if ( dwReason == DLL_PROCESS_DETACH ) {
		zncadDLL.DetachInstance () ;
	}
	return (TRUE) ;
}
