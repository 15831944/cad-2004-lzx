...
public:
	...
	vector<CGuiDaoNode> vec_GuiDaoInfo;
	vector<CDaocha>  vec_DaochaInfo;
	vector<CXhjDataNode> vec_XhjInfo;
...
// 	int m_iXgNum;
	...
	// 查找pt_3d_base所在位置的实体
	 ...
	 // 将信号机或绝缘节移至线上
	 void MoveXhjJyj(AcDbBlockReference *pRef, double dTol = 0.5) ;
	 ...
	 int DrawSxt_DT(double dScaleX, double dScaleY, const AcGePoint3d & pt_3d_orign, double dLenDcPlus=8.000, double dDistBetweenSX=2.500);
  	 BOOL m_bNetWithSn;
	 double GetPlineAngle(long nHandle,  double dAngleTo=0, int iStartOrEnd=0);
	 // 多段线起点或终点（iStartOrEnd=0，起点，=1，终点）
	 void GetPlinePoint(long nHandle, AcGePoint3d& pt_3d_ret, int iStartOrEnd=0) ;
...
//	void GetGuiDaoDuanXHJ(bool bforward,UINT nLineHandle, long& ZhiXHJ,long& XiangXHJ);
	...
	// void FindFirstDiffString(vector<CString>&vec_str1,vector<CString>&vec_str2,CString &str_diff);
	 ...
	 void testdtpmt();
	 int DrawSxt_DT2(double dScaleX, double dScaleY, const AcGePoint3d & pt_3d_orign, double dLenDcPlus, double dDistBetweenSX);
	 void DT_DrawBoxXhj(void);
	 void DT_DrawBoxXhj(long nHandleXhj);
	 void DT_DrawBoxXhj(long nHandleXhj,CXhDevice *pData);
	 void DT_DrawBoxJZ(void);
	 void DT_DrawBoxJZ(long nHandleXhj);
	 void DT_DrawBoxJZ(long nHandle,CXhDevice *pData);
	 void ModEntData_DT(void);
	 void DT_DrawSXDlk(long nHandle);
	 void DT_DrawSXDlk(void);
	// BOOL SelectFilePath2(CString &strFilePath);
	 BOOL SelectOpenFile(CString &strFilePath);
	 void DT_ComputeCableInfo(long nHandleCable);
	 void DT_ComputeCableInfo(long nHandleBoxStart, long nHandleBoxEnd, long nHandleCable, int iJigType);
	 void DT_InsertArrow(AcGePoint3d pt_3dInsert,int iType,bool bToRight,long nHandleCable);
	 void GetBoxLocation(long nHandleBox,CString &str_location,double d_tt=2.0);
	 int DT_GetCrossGd(long nHandleCable, CLongArray& nArrayHandleGd);
	 void GetBlockEnt_xAndUpDown(long nHandleBlk,double &d_ent_x,int &i_up);
	 void GetRFMBettwenTwoBox(long nHandleBoxStart,long nHandleBoxEnd,int &CntRfm);
	 void GetBlkLocation(long nHandleblk,CString &str_location1,CString &str_location2,double d_tt=3.0);
	// void GetXLocation(double sxt_ent_x,CString &str_location1,CString &str_location2,double d_tt=3.0);
	 void GetXLocation(int iUp,double sxt_ent_x,CString &str_location1,CString &str_location2,double d_tt=3.0);
	 void GetCDLianBettwenTwoBox(long nHandleBoxStart,long nHandleBoxEnd,double & d_cdLength);
	 void DT_GetZuoTouCount(long nHandleBoxStart,long nHandleBoxEnd,int &CntZT);
	 int DT_SetCableString(long nHandleCableLine, long nHandleCableString, CString& strCableString, int iStringType);
	// int DT_SetCableString(long nHandleCableLine, long nHandleCableString, CString& strCableString, int iStringType);
	 int DT_SetCableName(long nHandleCableLine, long nHandleCableName, CString& strCableName);
	 int DT_DrawCableString(long nHandleCableLine, const CString& strCableString, int iStringType);
	 int DT_GetCableString(long nHandleCableLine, long& nHandleCableString, int iType);
	 void DT_GetAnZhuangCount(long nHandleBoxStart,long nHandleBoxEnd,int &CntAZ);
	 int DT_FindCable(long nHandleBox, long nHandlePrePline, CLongArray& nArrayHandleFind);
	 int DT_FindCable(long nHandleBox, long nHandlePrePline, CLongArray& nArrayHandleFind,int iCableType);
	 void DT_ReadJltToDb(void);
	 void DT_FindBox(long nHandlePline, long nHandleNextBox, long nHandleBelongXhl, CString& strBranchIndex);
	 void DT_FindBox(long nHandlePline, long nHandleNextBox, sequential_tree<CBoxNodeEnt>* pSeqTree);
	 BOOL DT_GetBoxInfo(long nHandleBox, CBoxNodeEnt& boxNode);
	 int DT_GetCable(CLongArray (&nArrayHandle)[3]);
	 void DT_ReadJltToDbTest(void);
	 void DT_BuildCableBoxTree(long nHandleBox, long nHandleCable, sequential_tree<CBoxNodeEnt>& cableTree);
	 BOOL DT_FindCableCoreNumFromCableString(const CString strCableString,int &iNum_Cable,int &iNum_Back);
	 void DT_AddBox(void);
	// void DT_AddBox(void);
	 BOOL DT_GetUseCableType(int iUseCoreNum, int& iBackupNum, CString& strCableTypeName, int iDevType, int iCableType);
	 int DT_GetCableCoreNum(sequential_tree<CBoxNodeEnt>& cableBoxTree, CString& strCableTypeName, int& iBackup, int iDist);
	 void GetBoxLocationForPMT(long nHandleBox,CString &str_location,double d_tt);
	 void DT_DrawZzj(long nHandleDc);
	 //void DT_DrawBoxZzj(AcDbBlockReference* pRefJxg, const CDcDATA* pDataDc, int iQyIndex, int iDcDirectH);
	 void DT_DrawBoxZzj(AcDbBlockReference* pRefJxg, const CDtDcDATA* pDataDc, int iQyIndex, int iDcDirectH);
	 void DT_DrawBoxZzj(long nHandleDc);
	 BOOL DT_IsFromSameDevice(long nHandleDev1,long nHandleDev2);
	 int FindDaoChaHandleFromVector(long nHandle);
	 void testdtpmt_0();
	 void testdtpmt2();
	 void InitLsbDatabase();
	 int FindGuiDaoHandleFromVector( long nHandle );
	 BOOL FindLineType( long nHandleLineStart,int &iLineType);
	 //BOOL GetDaochaBlockPosition_X( long nHandleDC,double &d_x);
	 //BOOL FindDaocha_Byfwx( long nHandleLine_fw,long nHandleDC);
	 BOOL FindDaocha_Byfwx( long nHandleLine_fw,long &nHandleDC);
	 int FindXhjHandleFromVector( long nHandleXhj );
	 int GetXHJ_ConLineRight( long nHandleDC,long &nHandleLine);
	 int GetXHJ_ConLineLeft( long nHandleDC,long &nHandleLine);
	 void GetXhjEntName( long nHandle,CString &Entname );
	 void GetDaoChaEntName( long nHandle,CString &Entname );
	 BOOL GetXhjBlockPosition_X( long nHandle,double &point_x );
	 BOOL GetDaochaBlockPosition_X( long nHandle,double &point_x );
	 void GetXhjBlkNameByHandle( const long nHandle,CString &str_ent_name);
	 BOOL GetXhjBlockToward( long nHandleXHJ ,int &iToward);
	// void testdtpmt2();
	// BOOL DT_FindCableBackNumFromCableString(CString strCableString,int iNum_Back);
	// void DT_DrawBoxJZ(void);
	 };
