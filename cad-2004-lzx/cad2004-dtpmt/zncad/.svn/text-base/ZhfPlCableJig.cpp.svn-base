...
int CZhfPlCableJig::DT_doIt(void)
{
	int iRet = 0 ;
	CUserSystem mysys ;
	...
	//增加交点捕捉，6:10110001
	int i_Var=0x293;
	mysys.SetSysVar(_T("OSMODE"), i_Var) ; //1（端点）、2（中点、5（象限点）、8（垂足）,10010011 
	...
	do 
	{
		...
		if (stat==AcEdJig::kNormal)
		{
			if (iNumVert==0)
			{
				if (this->DT_GetPickBox(m_pt3dCur, m_nHandleBoxStart)==1)
				{
					m_pPolyLine->addVertexAt(iNumVert, m_pt3dCur.convert2d(AcGePlane::kXYPlane)) ;
					m_pPolyLine->addVertexAt(iNumVert+1, m_pt3dCur.convert2d(AcGePlane::kXYPlane)) ;
				}
			...
			else
			{
				...
				long nHandleBox = 0 ;
				if (this->DT_GetPickBox(m_pt3dCur, m_nHandleBoxEnd)==1)
				{
					if (m_nHandleBoxEnd==m_nHandleBoxStart)
					{
						AfxMessageBox(_T("径路起点和终点不能选在同一盒子！")) ;
...
// pick点所在的箱盒
int CZhfPlCableJig::DT_GetPickBox(const AcGePoint3d& pt_3d_pick, long& nHandleBox)
{
	//AcDbDatabase* pDb = acdbHostApplicationServices()->workingDatabase() ;
	CUserSystem mysys ;
	//mysys.m_pDb = pDb ;
	...
	long iLen = 0 ;
	CLongArray iArrayHandle ;
	CZnUtility::DT_FindConBox(pt_3d_pick, iArrayHandle, 0.1) ;
	iLen = iArrayHandle.GetCount() ;
	...
	if (iLen>0)
	{
		...
		if (mysys.OpenAcDbEntity(iArrayHandle.GetAt(0), pEnt, AcDb::kForRead))
		{
			if (pEnt->isKindOf(AcDbBlockReference::desc()))
			{
				...
				else if(strBlkName.Left(7)==_T("DT_XHJ_"))
				{
					nHandleBox = iArrayHandle.GetAt(0);
					iRet++;
				}
				...
				else if(strBlkName.Left(5)==_T("DT_AP"))
				{
					nHandleBox = iArrayHandle.GetAt(0) ;
					iRet++ ;
				}
