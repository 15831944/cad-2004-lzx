...
// 	CBlkUtility::ConvMultiToWide(szClassName, strClassName) ;
	...
	{
		nRow =19;
		strDlgTitle.Append(_T("-地铁道岔"));
	}
	else if(strClassName==_T("CCableImportDATA"))
	{
		nRow=5;
		strDlgTitle.Append(_T("-电缆孔"));
	}
	else if(strClassName==_T("CXhDevice"))
	{
		nRow=14;
		strDlgTitle.Append(_T("-轨旁设备"));
	}
...
void CEntPropertyDlg::OnBnClickedOk()
{
	...
	{
		CDtDcDATA *pDATA = dynamic_cast<CDtDcDATA*>(m_grid.m_pEntDATA) ;
		pDATA->m_strName = m_grid.QuickGetText(0, 0) ;
		m_grid.QuickGetNumber(0, 1, pDATA->m_nLoc) ;				
		m_grid.QuickGetNumber(0, 2, pDATA->m_nJG_QyNum) ;
		pDATA->m_strJG_QyType = m_grid.QuickGetText(0, 3) ;
		pDATA->m_strJG_QyPos = m_grid.QuickGetText(0, 4) ;
		pDATA->m_bDC_HasBox = m_grid.QuickGetBool(0,5);
		pDATA->m_bDC_ShareBox = m_grid.QuickGetBool(0,6);
		pDATA->m_strDC_BoxType = m_grid.QuickGetText(0,7);
		pDATA->m_strDC_BoxPos = m_grid.QuickGetText(0,8);
		pDATA->m_strCableString = m_grid.QuickGetText(0,9);
		pDATA->m_strBoxType = m_grid.QuickGetText(0, 10) ;
		pDATA->m_bIsSingleAct = m_grid.QuickGetBool(0, 11) ;
		pDATA->m_bIsComplex = m_grid.QuickGetBool(0, 12) ;
		pDATA->m_strRelationDc = m_grid.QuickGetText(0, 13) ;
	...
	{
		CCableImportDATA *pDATA = dynamic_cast<CCableImportDATA*>(m_grid.m_pEntDATA) ;
		pDATA->m_strStation = m_grid.QuickGetText(0, 0) ;
		CString strText = m_grid.QuickGetText(0, 1) ;
		pDATA->m_bIsJZ =(strText==_T("否")?0:1) ;		
		m_grid.QuickGetNumber(0, 2, pDATA->d_nLoc) ;	
		strText = m_grid.QuickGetText(0, 3) ;
		pDATA->m_bIsJZ =(strText==_T("否")?0:1) ;	
		m_grid.QuickGetNumber(0, 4, pDATA->m_dDistToDeviceRoom) ;
	}
	else if(strClassName==_T("CXhDevice"))
	{
		CXhDevice *pDATA=dynamic_cast<CXhDevice*>(m_grid.m_pEntDATA) ;
		pDATA->m_bHasGround=m_grid.QuickGetBool(0,13);
		pDATA->m_bHasGT=m_grid.QuickGetBool(0,12);
		pDATA->m_bHasPhone=m_grid.QuickGetBool(0,11);
		pDATA->m_strCable2=m_grid.QuickGetText(0,10);
		pDATA->m_strCable1=m_grid.QuickGetText(0,9);
		m_grid.QuickGetNumber(0,8,pDATA->m_nCoreNum2);
		m_grid.QuickGetNumber(0,7,pDATA->m_nCoreNum1);
		m_grid.QuickGetNumber(0,6,pDATA->m_nCable);
		CString strText2 = m_grid.QuickGetText(0, 5) ;
		pDATA->m_strBoxPos=strText2;
		pDATA->m_strBoxType=m_grid.QuickGetText(0,4);
		CString strText = m_grid.QuickGetText(0, 3) ;
		pDATA->m_bHasBox =(strText==_T("否")?0:1) ;	
        m_grid.QuickGetNumber(0,2,pDATA->m_dLoc);
		pDATA->m_strName=m_grid.QuickGetText(0,0);
