...
BOOL CCableNet::ComputeCoreAndDraw()
{
	...
	{				
		...
		if (cSet.Open(strSql))
		{
			...
			acedSetStatusBarProgressMeter(strBarMsg, 0, iRsCount) ;
			...
			while (!cSet.IsEOF())
			{
				...
				if (m_strSavePath.IsEmpty())
				{
					...
					CString strFileWithoutExt ;
					CBlkUtility::GetFileName(strFileCur, strFileWithoutExt) ;
					CString strStationDir ;
					strStationDir.Format(_T("%s\\%s"), m_strSavePath, strFileWithoutExt) ;
					if (CBlkUtility::IsFolderExist(strStationDir))
					{
						m_strSavePath = strStationDir ;
					}
					else
					{
						if(::CreateDirectory( strStationDir, NULL ))
						{
							m_strSavePath = strStationDir ;
						}
					}
...
//计算配线,一个如楼端一张图
BOOL CCableNet::DT_ComputeCoreAndDraw()
{
	BOOL bRet = TRUE ;
	CString strErrMsg ;
	try
	{				
		...
		if (cSet.Open(strSql))
		{
			...
			while (!cSet.IsEOF())
			{
				...
				CString strSaveFile ;
				strSaveFile.Format(_T("%s\\%s.dwg"), this->m_strSavePath, strFileName) ;		
				//绘制配线，输入文件所在路径和引入信号楼端电缆线；
				this->DT_DrawPx(strSaveFile, iXHL_handle_low,dCurTotalY) ;
				//dCurTotalY-=300;
...
//计算配线,一个如楼端一张图
BOOL CCableNet::DT_ComputeCoreAndDraw2()
{
	BOOL bRet = TRUE ;
	CString strErrMsg ;
	try
	{				
		...
		//////////////////////////////////////////////////////////////////////////
       //单拉的线
		strSql.Format(_T("SELECT * FROM cablenet_ent where boxtype='XHL_0' and devname='MAIN' ")) ;
		if (cSet.Open(strSql))
		{
			int iRsCount = cSet.GetRecordCount() ;
			...
			while (!cSet.IsEOF())
			{
				...
				cSet.GetFieldValue(_T("handle_low"), iXHL_handle_low) ;	
				CString strFileName = _T("未知");
				cSet.GetFieldValue(_T("devtype"), strFileName) ;
				int ileft_or_right=-1;
				cSet.GetFieldValue(_T("left_or_right"),ileft_or_right);
				long nHandleImport=0;
				cSet.GetFieldValue(_T("belong_xhl_handle_low"),nHandleImport);
				CString strnum;
				strnum.Format(_T("%d"),iIndex);
				strFileName+=strnum;
				//绘制编号为iIndex的配线图
				if (m_strSavePath.IsEmpty())
				{
				...
				strSaveFile.Format(_T("%s\\%s.dwg"), this->m_strSavePath, strFileName) ;		
				//绘制配线，输入文件所在路径和引入信号楼端电缆线；
				//this->DT_DrawPx3(strSaveFile,iXHL_handle_low,0,nHandleImport) ;
				this->DT_DrawDLPx3(strSaveFile,5366,5891,5292);
				//dCurTotalY-=300;
				iIndex++ ;
				cSet.MoveNext() ;
			}
...
//计算配线,一个如楼端一张图
BOOL CCableNet::DT_ComputeCoreAndDraw3()
{
	BOOL bRet = TRUE ;
	CString strErrMsg ;
	try
	{	
		...
		CString strFileName=_T("未知");
		this->DT_FindCableImport(nArrayHandleImport);
		int iCntCable=nArrayHandleImport.GetCount();
		CString strBarMsg ;
		strBarMsg.Format(_T("电缆配线:%d张"), 8) ;
		acedSetStatusBarProgressMeter(strBarMsg, 0, 8) ;
		if (m_strSavePath.IsEmpty())
		{
			AcApDocument* pDocCur = acDocManager->curDocument() ;
			CString strFileCur =  pDocCur->fileName() ;
		...
		//int iIndex=0;
		//查找某个电缆孔下各种类型设备
		CString strArrayDevType[8] = {_T("信号机"), _T("计轴"), _T("应答器"), _T("AP"), _T("转辙机"), _T("信号设备室"), _T("EMP"), _T("其他")} ;
		for(int j=0;j<8;j++)
		{
			...
			{
				CLongArray nArrayHandleMainLeft;
				CLongArray nArrayHandleMainRight;
				this->DT_FindMainCable(nArrayHandleImport.GetAt(i),strArrayDevType[j],nArrayHandleMainLeft,nArrayHandleMainRight);
				strFileName=strArrayDevType[j];
				CString strnum;
				strnum.Format(_T("_%d"),index);
				index++;
				strFileName+=strnum;
				CString strSaveFile ;
				strSaveFile.Format(_T("%s\\%s.dwg"), this->m_strSavePath, strFileName) ;	
				if(nArrayHandleMainLeft.GetCount()>0&&nArrayHandleMainRight.GetCount()>0)
				{
					this->DT_DrawDLPx3(strSaveFile,nArrayHandleMainLeft.GetAt(0),nArrayHandleMainRight.GetAt(0),nArrayHandleImport.GetAt(0));
				}
				else if(nArrayHandleMainLeft.GetCount()==0&&nArrayHandleMainRight.GetCount()>0)
				{
					this->DT_DrawDLPx3(strSaveFile,0,nArrayHandleMainRight.GetAt(0),nArrayHandleImport.GetAt(0));
				}
				else if(nArrayHandleMainRight.GetCount()==0&&nArrayHandleMainLeft.GetCount()>0)
				{
					this->DT_DrawDLPx3(strSaveFile,nArrayHandleMainLeft.GetAt(0),0,nArrayHandleImport.GetAt(0));
...
BOOL CCableNet::ReadAndFillDB(AcDbDatabase *pCurDb)
{
	BOOL bRet = TRUE ;
    //进度条调用函数，下次可以加到联锁表里
	acedSetStatusBarProgressMeter(_T("读取网络图..."), 0, 100) ;	
	...
	acedSetStatusBarProgressMeterPos(75) ;
	if (!this->FillDB())
	{
		return FALSE ;
	}
...
void CCableNet::DrawPx(CString strDwgFilePath, long lXHLHandle)
{
	try
	{
		...
		strBlockFile.Format(_T("%s\\support\\dz\\DZ_01.DWG"), m_strSysBasePath) ;
...
//绘制配线图，一个如楼端绘制一张图
void CCableNet::DT_DrawPx(CString strDwgFilePath, long lXHLHandle,double dYCur)
{
	try
	{
		AcDbDatabase *pCurDb = new AcDbDatabase() ;
		...
		CADORecordset cSet (&m_AdoDb) ;
		CString strSql ;
		int iLeftorRight=0;//1为左侧，0为右侧
		//查询信号设备室位于左侧还是右侧
		strSql.Format(_T("select * from cablenet_ent where handle_low=%d"),lXHLHandle);
		m_RecordSet.GetRecordValue(strSql,_T("left_or_right"),iLeftorRight);
		//获取电缆孔信息：信号设备室名称；是否集中站
		CString strStationName=_T("");
		long nHandleCableImport=0;
		mysys.GetXdata(lXHLHandle,_T("FATHER"),nHandleCableImport);
		mysys.GetXdata(nHandleCableImport,_T("STATIONNAME"),strStationName);
		if(strStationName==_T(""))
		{
			AfxMessageBox(_T("没有设置电缆孔所属车站"));
		}
// 	   ads_point pt_3d_test;
// 	   pt_3d_test[0]=0;
// 	   pt_3d_test[1]=0;
// 	   //pt_3d_test.z=0;
// 	   mysys.AddText(_T("Ceshi"),pt_3d_test,pt_3d_test,AcDb::kTextMid,AcDb::kTextVertMid);
		//开始绘制线
		mysys.m_pDb = pCurDb ;
		strSql.Format(_T("select * from cablenet_ent where belong_xhl_handle_low=%d order by branch_index desc"),lXHLHandle);
		if (cSet.Open(strSql))
		{
			double dCurTotalYForOneDwg = dYCur ; //绘制到一张图纸时，记录每个块绘制时的Y坐标		
			double dCurTotalXForOneDwg = 0;
			//当前已使用芯数和备用芯数总和；
			int iUseCore=0;
			//int iBackCore=0;
			strStationName+=_T("信号设备室");
			int iLevelLast=0;
			CString strLastDev=_T("");
			while (!cSet.IsEOF())
			{
				int iRsCount = cSet.GetRecordCount() ;
				CString strBranch_index;
				long nHandleNext=0;
				CString strCablestring;
				CString strBoxtype;
				CString strBoxBlock;
				cSet.GetFieldValue(_T("branch_index"), strBranch_index);
				cSet.GetFieldValue(_T("next_ent_handle_low"), nHandleNext);
				cSet.GetFieldValue(_T("cablestring"),strCablestring);
				cSet.GetFieldValue(_T("boxtype"),strBoxtype);
				cSet.GetFieldValue(_T("box_block"),strBoxBlock);
				//根据strBranch_index长度，设置起始X位置
				int iLevel=CBlkUtility::GetCharCountFromCString(strBranch_index,_T('-'));
				dCurTotalXForOneDwg=(iLeftorRight==0?-(iLevel+1)*m_dCellWidth:(iLevel+1)*m_dCellWidth);
				int iNumCore=0;
				int iNumBack=0;
				CZnSxt dtsxt;
				if(iLevel==iLevelLast)
				{
					//位于同一层
					//dCurTotalXForOneDwg=(iLeftorRight==0?m_dCellWidth:-m_dCellWidth);
					dtsxt.DT_FindCableCoreNumFromCableString(strCablestring,iNumCore,iNumBack);
					if(iNumCore<0||iNumBack<0)
					{
						AfxMessageBox(_T("设备电缆芯数和备用芯数有误"));
						return;
					}
					//判断是否为叶子节点
					if(strBoxBlock.Left(7)==_T("DT_BOX_"))
					{
						//为箱盒
						iLevelLast=iLevel;
						AfxMessageBox(_T("径路顺序有问题"));
						return ;
					...
					{
						//为叶子节点，即设备
						this->DT_DrawDZline(dCurTotalYForOneDwg-15,dCurTotalXForOneDwg,iLeftorRight,iLevel,iNumCore,iNumBack,strCablestring,pCurDb);
						//画框
						double dhight=10+(iNumCore-iNumBack)*5+10;
						this->DT_DrawPxFrame(dCurTotalXForOneDwg,dCurTotalYForOneDwg,iLeftorRight,strBoxtype,_T(""),dhight,pCurDb);
						//写设备名称
						strLastDev=strBoxtype;
						dCurTotalYForOneDwg-=dhight;
						iUseCore+=(iNumCore-iNumBack);
						iLevelLast=iLevel;
						//iBackCore+=iNumBack;
					}
				...
				else
				{
					...
					if(strBoxBlock.Left(7)==_T("DT_BOX_")) 
					{
						...
						int iNumCoreT=0;
						int iNumBackT=0;
						iNumCoreT=(iNumCore-iUseCore);
						iNumBackT=iNumBack;
						if(iNumCoreT<iNumBackT)
						{
							AfxMessageBox(_T("备用芯线数超过了可用的芯线数目"));
							//return ;
						}
						//绘制端子线
						//上移一个单元格子
						dCurTotalYForOneDwg+=m_dCellHight;
						this->DT_DrawDZline(dCurTotalYForOneDwg,dCurTotalXForOneDwg,iLeftorRight,iLevel,iNumCoreT,iNumBackT,strCablestring,pCurDb);
						//画框
						if(iNumCoreT>iNumBackT)
						{
							dCurTotalYForOneDwg-=((iNumCoreT-iNumBackT-1)*m_dCellHight+10);
						}
						else
						{
							dCurTotalYForOneDwg-=m_dCellHight;
						}
						double dhight=dYCur-dCurTotalYForOneDwg;
						this->DT_DrawPxFrame(dCurTotalXForOneDwg,dYCur,iLeftorRight,strLastDev,strBoxtype,dhight,pCurDb);
						//dCurTotalYForOneDwg-=dhight;
						//写设备名称
						//重新计算已使用芯线数
						iUseCore=(iNumCore-iNumBack);
			...
			//添加信号设备室方框
			this->DT_DrawPxSBSFrame(0,dYCur,dYCur-dCurTotalYForOneDwg,m_dCellWidth,iLeftorRight,strStationName,pCurDb);
			cSet.Close();
		}
		pCurDb->saveAs(strDwgFilePath) ;
		delete pCurDb ;
...
//绘制配线单元图，从离信号楼最远端至信号楼端
void CCableNet::DT_DrawDanlaPx(CString strDwgFilePath, long lCableHandle,int ileft_or_right,long nHandleCableImport,double dYCur)
{
	try
	{
		AcDbDatabase *pCurDb = new AcDbDatabase() ;
		...
		AcDbObjectId textStyleId = NULL ;
		textStyleId = mysys.CreateTextStyleByFileName(_T("ZhfDlpx"), _T("txt.shx"), _T("hztxt.shx"), 2, 0.8) ;
	    AcDbObjectId textStyleIdCH =NULL;
		textStyleIdCH=mysys.CreateTextStyle(_T("CHINESE"), _T("Times New Roman"), _T(""), 2.5, 0.8) ;
		CADORecordset cSet (&m_AdoDb) ;
		CString strSql ;
		int iLeftorRight=ileft_or_right;//1为左侧，0为右侧
		//查询信号设备室位于左侧还是右侧
		strSql.Format(_T("select * from cablenet_ent where handle_low=%d"),lCableHandle);
		CString strBranch_index;
		long nHandleNext=0;
		CString strCablestring;
		CString strBoxtype;
		CString strBoxBlock;
		m_RecordSet.GetFieldValue(_T("branch_index"), strBranch_index);
		m_RecordSet.GetFieldValue(_T("next_ent_handle_low"), nHandleNext);
		m_RecordSet.GetFieldValue(_T("cablestring"),strCablestring);
		m_RecordSet.GetFieldValue(_T("boxtype"),strBoxtype);
		m_RecordSet.GetFieldValue(_T("box_block"),strBoxBlock);
		//m_RecordSet.GetRecordValue(strSql,_T("left_or_right"),iLeftorRight);
		//获取电缆孔信息：信号设备室名称；是否集中站
		CString strStationName=_T("");
		mysys.GetXdata(lCableHandle,_T("CABLEIMPORT"),nHandleCableImport);
		mysys.GetXdata(nHandleCableImport,_T("STATIONNAME"),strStationName);
		if(strStationName==_T(""))
		{
			AfxMessageBox(_T("没有设置电缆孔所属车站"));
		...
		while(nHandleNext!=0)
		{
			//nHandleNext可能有多条记录
			//开始绘制线
			strSql.Format(_T("select * from cablenet_ent where handle_low=%d and is_start=1 order by pt_x asc"),lCableHandle);
			if (cSet.Open(strSql))
			{
				double dCurTotalYForOneDwg = dYCur ; //绘制到一张图纸时，记录每个块绘制时的Y坐标		
				double dCurTotalXForOneDwg = 0;
				...
				while (!cSet.IsEOF())
				{
					...
					CZnSxt dtsxt;
					//只考虑离信号楼最远一层的节点
					if(iLevel==iLevelLast||iLevelLast==0)
					{
						//位于同一层
						//dCurTotalXForOneDwg=(iLeftorRight==0?m_dCellWidth:-m_dCellWidth);
						dtsxt.DT_FindCableCoreNumFromCableString(strCablestring,iNumCore,iNumBack);
...
//绘制配线图，一个如楼端绘制一张图
void CCableNet::DT_DrawPx2(CString strDwgFilePath, long lXHLHandle,int left_or_right,long nHandleCableImport,double dYCur)
{
	try
	{
		AcDbDatabase *pCurDb = new AcDbDatabase() ;
		...
		CADORecordset cSet (&m_AdoDb) ;
		CString strSql ;
		int iLeftorRight=left_or_right;//1为左侧，0为右侧
		//查询信号设备室位于左侧还是右侧
		strSql.Format(_T("select * from cablenet_ent where belong_xhl_handle_low=%d"),lXHLHandle);
		m_RecordSet.GetRecordValue(strSql,_T("left_or_right"),iLeftorRight);
		//获取电缆孔信息：信号设备室名称；是否集中站
		CString strStationName=_T("");
		mysys.GetXdata(lXHLHandle,_T("CABLEIMPORT"),nHandleCableImport);
		mysys.GetXdata(nHandleCableImport,_T("STATIONNAME"),strStationName);
		if(strStationName==_T(""))
		{
			AfxMessageBox(_T("没有设置电缆孔所属车站"));
		...
		//开始绘制线
		strSql.Format(_T("select * from cablenet_ent where belong_xhl_handle_low=%d and is_start=1 order by pt_x asc"),lXHLHandle);
		if (cSet.Open(strSql))
		{
			double dCurTotalYForOneDwg = dYCur ; //绘制到一张图纸时，记录每个块绘制时的Y坐标		
			double dCurTotalXForOneDwg = 0;
...
//绘制配线图，一个如楼端绘制一张图
void CCableNet::DT_DrawPx3(CString strDwgFilePath, const long nHandleMainLineLeft,long const nHandleMainLineRight,const long nHandleImport)
{
	try
	{
		AcDbDatabase *pCurDb = new AcDbDatabase() ;
...
// 		//获取电缆孔信息：信号设备室名称；是否集中站
		...
		CString strName;
		long iIsup=1;
		//注意：获取扩展数据的时候，图形数据库一定要是当前打开的图纸，否则读取扩展数据会失败
		CUserSystem mysysCur;
		mysysCur.GetXdata(nHandleImport,_T("STATIONNAME"),strName);
		mysysCur.GetXdata(nHandleImport,_T("ISUP"),iIsup);
		strStationName=strName+_T("信号设备室");
		if(iIsup==0)
		{
			strStationName+=_T("(下行)");
		}
		else
		{
			strStationName+=_T("(上行)");
		}
		double dCurY=0;
		double dCurX=0;
		//标记访问过起始节点记录
		CLongArray nArrayHandleLineVistit;
		//开始绘制线
		strSql.Format(_T("select * from cablenet_ent where belong_xhl_handle_low=%d and is_start=1 order by pt_x asc"),nHandleMainLineLeft);
		if (cSet.Open(strSql))
		{
			double dCurTotalYForOneDwg = 0 ; //绘制到一张图纸时，记录每个块绘制时的Y坐标		
			double dCurTotalXForOneDwg = 0;
			...
			{
				//记录同一根线上的起始Y坐标
				double dCurTotalYForOneDwg2=dCurTotalYForOneDwg;
				CString strBranch_index;
				long nHandleNext=0;
				long nHandleCur=0;
				int iUseCore=0;
				...
				CString strBoxtype;
				CString strBoxBlock;
				CString strBackup_string;
				cSet.GetFieldValue(_T("handle_low"), nHandleCur);
				//nArrayHandleLineVistit.Add(nHandleCur);
				cSet.GetFieldValue(_T("next_ent_handle_low"), nHandleNext);
				//不是直接挂在主电缆上的线,只判断一次
				long nHandleNextCur=0;
				if(CBlkUtility::FindLongInArray(nArrayHandleLineVistit,nHandleCur)!=-1)
				{
					//continue;
					cSet.MoveNext();
					continue;
				}
				if(nHandleMainLineLeft!=nHandleNext&&nHandleNext!=0)
				{
					CString strSql2;
					strSql2.Format(_T("select * from cablenet_ent where belong_xhl_handle_low=%d and is_start=1 and next_ent_handle_low=%d order by pt_x asc"),nHandleMainLineLeft,nHandleNext);
					CADORecordset cSet2(&m_AdoDb);
					if(cSet2.Open(strSql2))
					{
						//同一层节点
						while (!cSet2.IsEOF())
						{
							int iRsCount=cSet2.GetRecordCount();
							CString strBranch_index;
							//long nHandleNext=0;
							long nHandleCur=0;
							CString strCablestring;
							...
							CString strBoxBlock;
							CString strBackup_string;
							cSet2.GetFieldValue(_T("handle_low"), nHandleCur);
						
							cSet2.GetFieldValue(_T("branch_index"), strBranch_index);
							cSet2.GetFieldValue(_T("next_ent_handle_low"), nHandleNextCur);
							cSet2.GetFieldValue(_T("cablestring"),strCablestring);
							cSet2.GetFieldValue(_T("backup_string"),strBackup_string);
							cSet2.GetFieldValue(_T("boxtype"),strBoxtype);
							cSet2.GetFieldValue(_T("box_block"),strBoxBlock);
							int iLevel=CBlkUtility::GetCharCountFromCString(strBranch_index,_T('-'));
							dCurTotalXForOneDwg=(iLeftorRight==0?-(iLevel+1)*m_dCellWidth:(iLevel+1)*m_dCellWidth);
							int iNumCore=0;
							int iNumBack=0;
							...
							double dhight=10+(iNumCore-iNumBack)*5+10;
							//dCurTotalYForOneDwg-=dhight;
							if(strBackup_string!=_T(""))
							{
								//为叶子节点，即设备
								this->DT_DrawDZline(dCurTotalYForOneDwg-15,dCurTotalXForOneDwg,iLeftorRight,iLevel,iNumCore,0,strCablestring,pCurDb);
								dhight+=((iNumCore)*5+10);
								dCurTotalYForOneDwg-=dhight;
							}
							//画框
							this->DT_DrawPxFrame(dCurTotalXForOneDwg,dCurTotalYForOneDwg,iLeftorRight,strBoxtype,_T(""),dhight,pCurDb);
						...
						cSet2.Close();
						//往上走一层
						nHandleNext=nHandleNextCur;
					}
				}
				else if(nHandleNext==nHandleMainLineLeft)
				{
					//设备和主缆直接相连
// 					if(CBlkUtility::FindLongInArray(nArrayHandleLineVistit,nHandleNext)!=-1)
// 					{
// 						break;
// 					}
					CString strSql2;
					strSql2.Format(_T("select * from cablenet_ent where belong_xhl_handle_low=%d and handle_low=%d order by pt_x asc"),nHandleMainLineLeft,nHandleCur);
					CString strCablestring;
					CString strBoxtype;
					CString strBoxBlock;
					CString strBackup_string;
					m_RecordSet.GetRecordValue(strSql2,_T("branch_index"), strBranch_index);
					m_RecordSet.GetRecordValue(strSql2,_T("next_ent_handle_low"), nHandleNextCur);
					m_RecordSet.GetRecordValue(strSql2,_T("cablestring"),strCablestring);
					m_RecordSet.GetRecordValue(strSql2,_T("backup_string"),strBackup_string);
					m_RecordSet.GetRecordValue(strSql2,_T("boxtype"),strBoxtype);
					m_RecordSet.GetRecordValue(strSql2,_T("box_block"),strBoxBlock);
					int iLevel=CBlkUtility::GetCharCountFromCString(strBranch_index,_T('-'));
					dCurTotalXForOneDwg=(iLeftorRight==0?-(iLevel+1)*m_dCellWidth:(iLevel+1)*m_dCellWidth);
					int iNumCore=0;
					int iNumBack=0;
					...
					CZnSxt dtsxt;
					dtsxt.DT_FindCableCoreNumFromCableString(strCablestring,iNumCore,iNumBack);
					int iNumCoreBack=iNumCore;
					int itemp=iNumCore-iNumBack;
					iNumCore-=iUseCore;
					iUseCore=itemp;
					if(iNumCore<0||iNumBack<0)
					{
					...
					{
						//为叶子节点，即设备
						this->DT_DrawDZline(dCurTotalYForOneDwg,dCurTotalXForOneDwg,iLeftorRight,iLevel,iNumCoreBack,0,strBackup_string,pCurDb);
						dhight+=((iNumCoreBack)*5+10);
						dCurTotalYForOneDwg-=dhight;
					}
					//dCurTotalYForOneDwg-=dhight;
					//画框
					this->DT_DrawPxFrame(dCurTotalXForOneDwg,dCurTotalYForOneDwg2,iLeftorRight,strBoxtype,_T(""),dCurTotalYForOneDwg2-dCurTotalYForOneDwg,pCurDb);
					//写设备名称
					strLastDev=strBoxtype;
					//dCurTotalYForOneDwg-=dhight;
					iUseCore+=(iNumCore-iNumBack);
					//nArrayHandleLineVistit.Add(nHandleNext);
					nHandleNext=nHandleNextCur;
					//cSet.MoveNext();
				}
					//记录同一根线最下方位置
					double dCurTotalYForOneDwg3=dCurTotalYForOneDwg;
					while (nHandleNext!=nHandleMainLineLeft&&nHandleNext!=0)
					{
						if(CBlkUtility::FindLongInArray(nArrayHandleLineVistit,nHandleNext)!=-1)
						{
							break;
						...
						CString strSql2;
						strSql2.Format(_T("select * from cablenet_ent where belong_xhl_handle_low=%d and handle_low=%d order by pt_x asc"),nHandleMainLineLeft,nHandleNext);
						CString strCablestring;
						CString strBoxtype;
						CString strBoxBlock;
						CString strBackup_string;
						...
						//为叶子节点，即设备
						this->DT_DrawDZline(dCurTotalYForOneDwg3+5,dCurTotalXForOneDwg,iLeftorRight,iLevel,iNumCore,iNumBack,strCablestring,pCurDb);
						double dhight=(iNumCore-iNumBack)*5;
						dCurTotalYForOneDwg-=dhight;
						if(strBackup_string!=_T(""))
						{
							//为叶子节点，即设备
				...
				//信号设备室左侧绘制完毕了。可以开始绘制右侧设备了
			//添加信号设备室方框
			this->DT_DrawPxSBSFrame(0,0,0-dCurTotalYForOneDwg,m_dCellWidth+30,iLeftorRight,strStationName,pCurDb);
			cSet.Close();
		}
		pCurDb->saveAs(strDwgFilePath) ;
		delete pCurDb ;
...
//绘制配线图，一个如楼端绘制一张图
void CCableNet::DT_DrawDLPx3(CString strDwgFilePath, const long nHandleMainLineLeft,long const nHandleMainLineRight,const long nHandleImport)
{
	try
	{
		AcDbDatabase *pCurDb = new AcDbDatabase() ;
		...
		double dYMaxleft=0;
		double dYMaxRight=0;
		double dXhSBSWidth=80;
		//先绘制信号设备室左侧
		this->DT_DrawDanLanPX(strDwgFilePath,dCurX,dCurY,dYMaxleft,nHandleMainLineLeft,0,pCurDb);
		//再绘制信号设备室右侧
		this->DT_DrawDanLanPX(strDwgFilePath,dCurX,dCurY,dYMaxRight,nHandleMainLineRight,1,pCurDb);
		double dHight=(fabs(dYMaxleft)<fabs(dYMaxRight)?fabs(dYMaxRight):fabs(dYMaxleft));
		this->DT_DrawPxSBSFrame(dCurX,dCurY,dHight,dXhSBSWidth,0,strStationName,pCurDb);
// 		if(nHandleMainLineLeft==0)
// 		{
// 			dXhSBSWidth=50;
// 			this->DT_DrawPxSBSFrame(80-dCurX,dCurY,dHight,dXhSBSWidth,0,strStationName,pCurDb);
// 		}
// 		else if(nHandleMainLineRight==0)
// 		{
// 			this->DT_DrawPxSBSFrame(dCurX-80,dCurY,dHight,dXhSBSWidth,1,strStationName,pCurDb);
// 		}
// 		else
// 		{
// 			dXhSBSWidth=80;
...
//从Cablenet_ent中找出连接有主电缆的电缆孔
void CCableNet::DT_FindCableImport(CLongArray &nArrayHandleImport)
{
	CString strSql;
	CADORecordset cSet(&m_AdoDb);
	//开始绘制线
	strSql.Format(_T("select * from cablenet_ent where devname='MAIN' and boxtype='XHL_0' "));
	if (cSet.Open(strSql))
	{
		while (!cSet.IsEOF())
		{
			long nHandle_low=0;
			cSet.GetFieldValue(_T("belong_xhl_handle_low"),nHandle_low);
			if(nHandle_low!=0)
			{
				if(CBlkUtility::FindLongInArray(nArrayHandleImport,nHandle_low)==-1)
				{
					nArrayHandleImport.Add(nHandle_low);
				}
			}
			cSet.MoveNext();
		}
...
//找出属于电缆孔的连接devtype的所有左线和有线
void CCableNet::DT_FindMainCable(long nHandleImport,CString strDevType,CLongArray &nArrayHandleLeft,CLongArray &nArrayHandleRight)
{
	CString strSql;
	CADORecordset cSet(&m_AdoDb);
	if(nHandleImport==0)
	...
	//开始绘制线
	strSql.Format(_T("select * from cablenet_ent where devname='MAIN' and boxtype='XHL_0' and devtype='%s ' and belong_xhl_handle_low=%d"),strDevType,nHandleImport);
	if (cSet.Open(strSql))
	{
		while (!cSet.IsEOF())
		{
			long nHandle_low=0;
			int iLeft_or_right=0;
			cSet.GetFieldValue(_T("handle_low"),nHandle_low);
			cSet.GetFieldValue(_T("left_or_right"),iLeft_or_right);
			if(nHandle_low!=0)
			{
				if(iLeft_or_right==0&&CBlkUtility::FindLongInArray(nArrayHandleLeft,nHandle_low)==-1)
				{
					nArrayHandleLeft.Add(nHandle_low);
				}
				else if(iLeft_or_right==1&&CBlkUtility::FindLongInArray(nArrayHandleRight,nHandle_low)==-1)
				{
					nArrayHandleRight.Add(nHandle_low);
				}
			}
			cSet.MoveNext();
		}
...
void CCableNet::DT_DrawDanLanPX(CString strDwgFilePath,double dCurX,double dCurY,double & dCurDWGTotalY,long nHandleMainLine,int iLeftorRight,AcDbDatabase *pCurDb)
{
	...
	CADORecordset cSet(&m_AdoDb);
	//开始绘制线
	strSql.Format(_T("select * from cablenet_ent where belong_xhl_handle_low=%d and is_start=1 order by pt_x asc"),nHandleMainLine);
	if (cSet.Open(strSql))
	{
		double dCurTotalYForOneDwg = dCurY ; //绘制到一张图纸时，记录每个块绘制时的Y坐标		
		double dCurTotalXForOneDwg = dCurX;
		//当前已使用芯数和备用芯数总和；
		//int iBackCore=0;
		//int iUseCore=0;
		//strStationName+=_T("信号设备室");
		...
		while (!cSet.IsEOF())
		{
			...
			if(nHandleMainLine!=nHandleNext&&nHandleNext!=0)
			{
				CString strSql2;
				strSql2.Format(_T("select * from cablenet_ent where belong_xhl_handle_low=%d and is_start=1 and next_ent_handle_low=%d order by pt_x asc"),nHandleMainLine,nHandleNext);
				CADORecordset cSet2(&m_AdoDb);
				if(cSet2.Open(strSql2))
				{
					//同一层节点
					...
					int iRsCount=0;
					int iCnt=0;
					double dCurYLevel=dCurTotalYForOneDwg;
					while (!cSet2.IsEOF())
					{
						//可能有多条尾缆
						iRsCount=cSet2.GetRecordCount();
						int iNumCore=0;
						int iNumBack=0;
						if(iRsCount>1)
						{
						...
						{
							this->DT_DrawDZline(dCurTotalYForOneDwg-15,dCurTotalXForOneDwg,iLeftorRight,iLevel,iNumCore,iNumBack,strCablestring,pCurDb);
							dhight=(10+(iNumCore-iNumBack)*5+10);
						}
						else
						{
							this->DT_DrawDZline(dCurTotalYForOneDwg-5,dCurTotalXForOneDwg,iLeftorRight,iLevel,iNumCore,iNumBack,strCablestring,pCurDb);
							dhight=(iNumCore-iNumBack)*5+10;
						}
// 							dhight=(iNumCore-iNumBack)*5+10;
						//dCurTotalYForOneDwg-=dhight;
						if(strBackup_string!=_T(""))
						{
							//为叶子节点，即设备
							this->DT_DrawDZline(dCurTotalYForOneDwg-dhight,dCurTotalXForOneDwg,iLeftorRight,iLevel,iNumCore,0,strCablestring,pCurDb);
							dhight+=((iNumCore-1)*5+10);
							//dCurTotalYForOneDwg-=dhight;
						}
						//画框
						if(iRsCount==1)
					...
					if(iRsCount>1)
					{
						this->DT_DrawPxFrame(dCurTotalXForOneDwg,dCurYLevel,iLeftorRight,strBoxtype,_T(""),dCurYLevel-dCurTotalYForOneDwg,pCurDb);
						//写设备名称
						// strLastDev=strBoxtype;
						//dCurTotalYForOneDwg-=dhight;
						//nArrayHandleLineVistit.Add(nHandleCur);
			...
			else if(nHandleNext==nHandleMainLine)
			{
				//设备和主缆直接相连,如AP
				// 					if(CBlkUtility::FindLongInArray(nArrayHandleLineVistit,nHandleNext)!=-1)
				// 					{
				...
				// 					}
				CString strSql2;
				strSql2.Format(_T("select * from cablenet_ent where belong_xhl_handle_low=%d and handle_low=%d order by pt_x asc"),nHandleMainLine,nHandleCur);
				CString strCablestring;
				CString strBoxtype;
				CString strBoxBlock;
				CString strBackup_string;
				...
				{
					//为叶子节点，即设备
					this->DT_DrawDZline(dCurTotalYForOneDwg-dhight,dCurTotalXForOneDwg,iLeftorRight,iLevel,iNumCoreBack,0,strBackup_string,pCurDb);
					dhight+=((iNumCoreBack-1)*5+10);
					//dCurTotalYForOneDwg-=dhight;
				}
				//dCurTotalYForOneDwg-=dhight;
				//画框
				this->DT_DrawPxFrame(dCurTotalXForOneDwg,dCurTotalYForOneDwg2,iLeftorRight,strBoxtype,_T(""),dhight,pCurDb);
				//写设备名称
				strLastDev=strBoxtype;
				dCurTotalYForOneDwg-=dhight;
				iUseCore+=(iNumCore-iNumBack);
			...
			//记录同一根线最下方位置
			double dCurTotalYForOneDwg3=dCurTotalYForOneDwg;
			while (nHandleNext!=nHandleMainLine&&nHandleNext!=0)
			{
				if(CBlkUtility::FindLongInArray(nArrayHandleLineVistit,nHandleNext)!=-1)
				{
					break;
				...
				CString strSql2;
				strSql2.Format(_T("select * from cablenet_ent where belong_xhl_handle_low=%d and handle_low=%d order by pt_x asc"),nHandleMainLine,nHandleNext);
				CString strCablestring;
				CString strBoxtype;
				CString strBoxBlock;
				CString strBackup_string;
				...
				{
					//为叶子节点，即设备
					this->DT_DrawDZline(dCurTotalYForOneDwg3-dhight,dCurTotalXForOneDwg,iLeftorRight,iLevel,iNumCoreBack,0,strBackup_string,pCurDb);
					dhight+=((iNumCoreBack-1)*5+10);
					//dCurTotalYForOneDwg-=dhight;
				}
				dCurTotalYForOneDwg-=dhight;
				//画框
				this->DT_DrawPxFrame(dCurTotalXForOneDwg,dCurTotalYForOneDwg2,iLeftorRight,strLastDev,strBoxtype,dCurTotalYForOneDwg2-dCurTotalYForOneDwg,pCurDb);
				//写设备名称
				strLastDev=strBoxtype;
				//dCurTotalYForOneDwg-=dhight;
				iUseCore+=(iNumCore-iNumBack);
				...
				nHandleNext=nHandleNextCur;
				//上一层Y值
				dCurTotalYForOneDwg3=dCurTotalYForOneDwg;
			}//end while 
			cSet.MoveNext();
		}//end while
		//信号设备室左侧绘制完毕了。可以开始绘制右侧设备了
		//添加信号设备室方框
		//this->DT_DrawPxSBSFrame(dCurX,dCurY,0-dCurTotalYForOneDwg,m_dCellWidth+30,iLeftorRight,_T("XX"),pCurDb);
		dCurDWGTotalY=dCurTotalYForOneDwg;;
		cSet.Close();
	}
}
...
// 					//end 轨道电路的盒子，送受电端子不用1~4（留给电码化使用），刘汉禹，京广
					...
					//begin 20150814：蔡菁华和刘汉禹pk，轨道电路和电码化端子，2线制时，送电端用1、3，1-10不用；受电端用1、2,1-10不用。4线制时，送电端用1、3,1-10不用，受电端用1、2,1-10不用，电码化，送受电用5、6,1-10不用。
					if (strDevType.Left(3)==_T("FSD"))
					{
						if (m_i4Or2Line==0&&strDevType.Left(6)==_T("FSD_DM")) //4线制且电码化，送受都用5、6,1-10不用
						{
							strSql.Format(_T("select * from define_box_detail where box_type_id=%d and (val(box_node)<5 or (val(box_node)>6 and  val(box_node)<11))"), iBoxTypeId) ;
						}
						else if (strDevType.Right(1)==_T("F")) // 送
						{
							strSql.Format(_T("select * from define_box_detail where box_type_id=%d and ((val(box_node)>3 and  val(box_node)<11) or box_node='2' )"), iBoxTypeId) ;
						}
						else if (strDevType.Right(1)==_T("S")) // 受
						{
							strSql.Format(_T("select * from define_box_detail where box_type_id=%d and (val(box_node)>2 and  val(box_node)<11) "), iBoxTypeId) ;
						}
						m_RecordSet.SetRecordValue(strSql, _T("is_used"), 1) ;
					}

					//end 20150814：蔡菁华和刘汉禹pk，轨道电路和电码化端子，2线制时，送电端用1、3，1-10不用；受电端用1、2,1-10不用。4线制时，送电端用1、3,1-10不用，受电端用1、2,1-10不用，电码化，送受电用5、6,1-10不用。
...
int CCableNet::ComputeCoreOfOneBox(long iBoxHandle)
{
	try
	{
		...
		//取得电缆定义的id
		int iCableTypeId = -1 ;
		int iLenOfCableStr = strCabeStrTmp2.GetLength() ;
		for (int i=0; i<iLenOfCableStr; i++) //例8BL,从后往前逐步删除字符，直到找到匹配的电缆定义为止
		{
			strSql.Format(_T("select id from define_cable_type where cable_name='%s'"), strCabeStrTmp2.Left(iLenOfCableStr-i)) ;
			if (m_RecordSet.GetRecordValue(strSql, _T("id"), iCableTypeId)==1)
			{
				break;
			}
		}
...
void CCableNet::DT_DrawDZline( const double dY_start,const double dX_Start,const int iLeft_or_Right,const int iLevel,const int iNumCore,const int iNumBack,CString strCableString,AcDbDatabase *pCurDb/*=acdbHostApplicationServices()->workingDatabase()*/)
{
	CUserSystem mysis;
	mysis.m_pDb=pCurDb;
	ads_point pt_base_text = {0,0,0} ;
	AcDbObjectId textStyleId = NULL ;
	textStyleId = mysis.CreateTextStyleByFileName(_T("ZhfDlpx"), _T("txt.shx"), _T("hztxt.shx"), 2, 0.8) ;
	AcDbObjectId textStyleIdCH =NULL;
	textStyleIdCH=mysis.CreateTextStyle(_T("CHINESE"), _T("Times New Roman"), _T(""), 2.5, 0.8) ;
	double d_x=dX_Start;
	double d_y=dY_start;
	if(iLeft_or_Right==0)
	{
		//d_x=dX_Start-(iLevel+1)*m_dCellWidth;
		d_x+=m_dCellWidth/2;
	}
	else 
	{
	...
	CString strBlockFile;
	strBlockFile.Format(_T("%s\\support\\dz\\DZ_01.DWG"), m_strSysBasePath) ;
	int iCntLine=iNumCore-iNumBack;
	int iCntLevel=iLevel;
	double d_hight=(iCntLine+2)*m_dCellHight+10;
	//d_y-=15;
	while (iCntLine>0)
	{
		...
		pt_3d_end.x=d_x+(iLeft_or_Right==0?m_dCellWidth:-m_dCellWidth);
		pt_3d_end.y=d_y;
		iCntLevel=iLevel;
		while(iCntLevel>0)
		{
			AcDbBlockReference *pRef = NULL;
			//CUserSystem mysis;
			if(mysis.InsertBlock(pRef, strBlockFile, pt_3d_start,0, 1))
			{
				pRef->close() ;
			}
			//int iColorIndex = (iIsAdd==0?0:1) ;					
			mysis.AddAcDbLine(pt_3d_start, pt_3d_end, 0) ;
			pt_3d_start=pt_3d_end;
			pt_3d_end.x+=(iLeft_or_Right==0?m_dCellWidth:-m_dCellWidth);
			iCntLevel--;
		}
		//在最后添加一个端子
		AcDbBlockReference *pRef = NULL;
	...
	//最后添加备用芯线信息
	if (iNumBack==iNumBack)
	{
		AcGePoint3d pt_3d_start;
		AcGePoint3d pt_3d_end;
		pt_3d_start.x = d_x+(iLeft_or_Right==0?(m_dCellWidth/2-10):(-m_dCellWidth/2+5)) ;
		pt_3d_start.y = d_y;
		pt_3d_end.x = pt_3d_start.x +(iLeft_or_Right==0?(15):(-15)) ; 
		pt_3d_end.y = pt_3d_start.y ;
		mysis.AddAcDbLine(pt_3d_start, pt_3d_end) ;
		int iPlusX=8;
		pt_base_text[0] = (iLeft_or_Right==0?(pt_3d_start.x+ iPlusX):(pt_3d_end.x+iPlusX))  ;
		pt_base_text[1] = pt_3d_start.y + 1 ; 
		AcDb::TextHorzMode iHorMode =AcDb::kTextRight ;	
		//CString strCableString;
		//strCableString.Format(_T("120-%d(%d)"),iNumCore,iNumBack);
		mysis.AddText(strCableString,pt_base_text, pt_base_text, iHorMode, AcDb::kTextBase, 2, 0.8, 0, 0, textStyleIdCH) ;
		if (1==1)
		{
			pt_base_text[1] -= 5 ;
			mysis.AddText(_T("备用:"), pt_base_text, pt_base_text, iHorMode, AcDb::kTextBase, 2, 0.8, 0, 0, textStyleIdCH) ;
			//pt_base_text[0] =pt_3d_start.x+((iLeft_or_Right==0)?12:-12);
			CString strBackupString=_T("D-5~6");
			pt_base_text[0]=(iLeft_or_Right==0?pt_3d_start.x+12:pt_3d_end.x+12) ; 
			AcDb::TextHorzMode iHorMode2 =AcDb::kTextLeft ;	
			mysis.AddText(strBackupString, pt_base_text, pt_base_text, iHorMode2, AcDb::kTextBase, 2, 0.8, 0, 0, textStyleIdCH) ;
		}
	}
...
void CCableNet::DT_DrawPxFrame( double dXstart,double dYstart,const int iLeft_or_Right,CString strDevName,CString strBoxName,double dCellHight,AcDbDatabase *pCurDb/*=acdbHostApplicationServices()->workingDatabase()*/ )
{
	ads_point pt_base_text = {0,0,0} ;
	pt_base_text[1] = dYstart-m_dCellHight ;
	pt_base_text[0] = (iLeft_or_Right==0?(dXstart+4):(dXstart-4)) ;
	CUserSystem mysys;
	mysys.m_pDb=pCurDb;
	AcDbObjectId textStyleIdCH =NULL;
	textStyleIdCH=mysys.CreateTextStyle(_T("CHINESE"), _T("Times New Roman"), _T(""), 2.5, 0.8) ;
	AcDb::TextHorzMode iHorMode1 =iLeft_or_Right==0?AcDb::kTextLeft:AcDb::kTextRight ;	
	mysys.AddText(strDevName, pt_base_text, pt_base_text, iHorMode1, AcDb::kTextVertMid, 2, 0.8, 0, 0,textStyleIdCH) ;
	pt_base_text[0] = (iLeft_or_Right==0?(dXstart+m_dCellWidth-4):(dXstart-m_dCellWidth+4));
	AcDb::TextHorzMode iHorMode2 =iLeft_or_Right==0?AcDb::kTextRight:AcDb::kTextLeft ;	
	mysys.AddText(strBoxName, pt_base_text, pt_base_text,iHorMode2, AcDb::kTextVertMid, 2, 0.8, 0, 0,textStyleIdCH) ;
	//绘制边框
	AcGePoint3d pt_3d_start;
	AcGePoint3d pt_3d_end;
	pt_3d_start.z=pt_3d_end.z=0;
	pt_3d_start.x = dXstart ;
	pt_3d_start.y = dYstart;
	pt_3d_end.x = pt_3d_start.x+(iLeft_or_Right==0?m_dCellWidth:-m_dCellWidth) ;
	pt_3d_end.y = pt_3d_start.y ;
	mysys.AddAcDb2dPolyline(pt_3d_start, pt_3d_end, 0.5) ;
	pt_3d_start.y-=10;
	pt_3d_end.y-=10;
	mysys.AddAcDb2dPolyline(pt_3d_start, pt_3d_end, 0.5) ;
	pt_3d_start.y=dYstart-dCellHight;
	pt_3d_end.y=dYstart-dCellHight;
	mysys.AddAcDb2dPolyline(pt_3d_start, pt_3d_end, 0.5) ;
	//绘制竖线
	pt_3d_start.x=dXstart;
	pt_3d_start.y=dYstart;
	pt_3d_end.x=pt_3d_start.x;
	pt_3d_end.y=pt_3d_start.y-dCellHight;
	mysys.AddAcDb2dPolyline(pt_3d_start, pt_3d_end, 0.5) ;
// 	pt_3d_start.x+=(iLeft_or_Right==0?m_dCellWidth:-m_dCellWidth);
// 	pt_3d_end.x=pt_3d_start.x;
// 	mysys.AddAcDb2dPolyline(pt_3d_start, pt_3d_end, 0.5) ;
}
...
void CCableNet::DT_DrawPxSBSFrame( double dXstart,double dYstart,double dhight,double dCellWidth,const int iLeft_or_Right,CString strSBSName,AcDbDatabase *pCurDb/*=acdbHostApplicationServices()->workingDatabase()*/ )
{
	ads_point pt_base_text = {0,0,0} ;
	ads_point pt_base_text2 = {0,0,0} ;
	pt_base_text[1] = dYstart;
	pt_base_text[0] = (iLeft_or_Right==0?(dXstart-m_dCellWidth):dXstart+dCellWidth-m_dCellWidth);
	pt_base_text2[1] = dYstart-m_dCellHight;
	pt_base_text2[0] = pt_base_text[0]+dCellWidth/2;
	CUserSystem mysys;
	mysys.m_pDb=pCurDb;
	AcDbObjectId textStyleIdCH =NULL;
	textStyleIdCH=mysys.CreateTextStyle(_T("CHINESE"), _T("Times New Roman"), _T(""), 2.5, 0.8) ;
	mysys.AddText(strSBSName, pt_base_text2, pt_base_text2, AcDb::kTextMid, AcDb::kTextVertMid, 2, 0.8, 0, 0,textStyleIdCH) ;
	//绘制边框
	AcGePoint3d pt_3d_start;
	AcGePoint3d pt_3d_end;
	pt_3d_start.z=pt_3d_end.z=0;
	pt_3d_start.x = pt_base_text[0];
	pt_3d_start.y = pt_base_text[1];
	pt_3d_end.x = pt_3d_start.x+dCellWidth;
	pt_3d_end.y = pt_3d_start.y ;
	mysys.AddAcDb2dPolyline(pt_3d_start, pt_3d_end, 0.5) ;
	pt_3d_start.y-=10;
	pt_3d_end.y-=10;
	mysys.AddAcDb2dPolyline(pt_3d_start, pt_3d_end, 0.5) ;
	pt_3d_start.y=pt_base_text[1]-dhight;
	pt_3d_end.y=pt_base_text[1]-dhight;
	mysys.AddAcDb2dPolyline(pt_3d_start, pt_3d_end, 0.5) ;
	//绘制竖线
	pt_3d_start.x=pt_base_text[0];
	pt_3d_start.y=pt_base_text[1];
	pt_3d_end.x=pt_3d_start.x;
	pt_3d_end.y=pt_3d_start.y-dhight;
	mysys.AddAcDb2dPolyline(pt_3d_start, pt_3d_end, 0.5) ;
	pt_3d_start.x+=dCellWidth;
	pt_3d_end.x+=dCellWidth;
	mysys.AddAcDb2dPolyline(pt_3d_start, pt_3d_end, 0.5) ;
}
