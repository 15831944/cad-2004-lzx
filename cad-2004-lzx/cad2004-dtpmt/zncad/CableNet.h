...
public:
	...
	BOOL DT_ComputeCoreAndDraw();
	//void DT_DrawPx(CString strDwgFilePath, long lXHLHandle);
	void DT_DrawPx(CString strDwgFilePath, long lXHLHandle,double dYCur);
	//void DT_DrawDZline( const double dY_start,const double dX_Start,const int iLeft_or_Right,const int iLevel,const int iNumCore,const int iNumBack,CString strCableString);
	void DT_DrawDZline( const double dY_start,const double dX_Start,const int iLeft_or_Right,const int iLevel,const int iNumCore,const int iNumBack,CString strCableString,AcDbDatabase *pCurDb=acdbHostApplicationServices()->workingDatabase());
	void DT_DrawPxFrame(double dXstart,double dYstart,const int iLeft_or_Right,CString strDevName,CString strBoxName,double dCellHight,AcDbDatabase *pCurDb=acdbHostApplicationServices()->workingDatabase());
	void DT_DrawPxSBSFrame( double dXstart,double dYstart,double dhight,double dCellWidth,const int iLeft_or_Right,CString strSBSName,AcDbDatabase *pCurDb=acdbHostApplicationServices()->workingDatabase());
	BOOL DT_ComputeCoreAndDraw2();
	//void DT_DrawPx2(CString strDwgFilePath, long lXHLHandle,double dYCur);
	void DT_DrawPx2(CString strDwgFilePath, long lXHLHandle,int left_or_right,long nHandleCableImport,double dYCur);
	void DT_DrawDanlaPx(CString strDwgFilePath, long lCableHandle,int ileft_or_right,long nHandleCableImport,double dYCur);
	//void DT_DrawPx3(CString strDwgFilePath, long nHandleMainLineLeft,long nHandleMainLineRight);
	void DT_DrawPx3(CString strDwgFilePath, const long nHandleMainLineLeft,long const nHandleMainLineRight,const long nHandleImport);
	//void DT_DrawDanLanPX(CString strDwgFilePath,double dCurX,double dCurY,long nHandleMainLine,int iLeftorRight,AcDbDatabase *pCurDb);
	void DT_DrawDanLanPX(CString strDwgFilePath,double dCurX,double dCurY,double &dCurDWGTotalY,long nHandleMainLine,int iLeftorRight,AcDbDatabase *pCurDb);
	void DT_DrawDLPx3(CString strDwgFilePath, const long nHandleMainLineLeft,long const nHandleMainLineRight,const long nHandleImport);
	void DT_FindCableImport(CLongArray &nArrayHandleImport);
	void DT_FindMainCable(long nHandleImport,CString strDevType,CLongArray &nArrayHandleLeft,CLongArray &nArrayHandleRight);
	BOOL DT_ComputeCoreAndDraw3();
	//void DT_DrawDLPx3(CString strDwgFilePath, const long nHandleMainLineLeft,long const nHandleMainLineRight,const long nHandleImport);
	int m_iDsNumXhj;
	int m_iQkNum;
	// 调车灯丝
