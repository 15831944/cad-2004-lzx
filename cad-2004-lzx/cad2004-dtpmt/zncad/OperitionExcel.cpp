...
BOOL COperitionExcel::ReadDatafromExcel( int iNum_sheet , int iRow , int iCol , double &dData )
{
	try
	{
		dData=0;
		...
		pRange = pSheet->GetRange((_variant_t)strCellName, vtMissing) ;
		//取得文本 ;
		_variant_t range_var = pRange->GetValue2();	
		dData = (double)(range_var.dblVal);
		return TRUE;
	}
	catch(_com_error &e)
	{
...
BOOL COperitionExcel::ReadDatafromExcel( int iNum_sheet,int iRow,int iCol,int &iData )
{
	try
	{
		VARIANT vt_index ;
		...
		//取得文本 ;
		_variant_t range_var = pRange->GetValue2();	
		iData = static_cast<int> (range_var.dblVal) ;
		return TRUE;
	}
	catch(_com_error &e)
	{
...
BOOL COperitionExcel::IsCellDouble( int iNum_sheet,int iRow,int iCol )
{
	try
	{
		VARIANT vt_index ;
		...
		//取得文本 ;
		_variant_t range_var = pRange->GetValue2();	
		if(range_var.vt == VT_DECIMAL||range_var.vt==VT_R8)       
		{
			return TRUE;
		}
		else
	...
	{	
		//iFound = 0 ;
		AfxMessageBox(_T("未知的错误!in IsCellDouble"));			
		return FALSE;
	}	
	return FALSE;
...
BOOL COperitionExcel::IsCellCString( int iNum_sheet,int iRow,int iCol )
{
	try
	{
		VARIANT vt_index ;
	...
	{	
		//iFound = 0 ;
		AfxMessageBox(_T("未知的错误!in IsCellString"));			
		return FALSE;
	}	
	return FALSE;
}

BOOL COperitionExcel::IsCellInt( int iNum_sheet,int iRow,int iCol )
{
	try
	{
		VARIANT vt_index ;
	...
	{	
		//iFound = 0 ;
		AfxMessageBox(_T("未知的错误!in IsCellInt"));			
		return FALSE;
	}	
	return FALSE;
