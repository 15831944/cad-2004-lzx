...
public:
 ...
 static void draw_shnicao(const int count ,const bool up,const AcGePoint3d startpoint,const AcGePoint3d endpoint);
  ...
  //static void drawpodu(const double d_xstart,const double d_xend,const double d_y,CString strbiao,CString str_polength,CString str_podu,bool up);
   void drawpodu( const AcGePoint2d pt2d,const double d_xend,CString strbiao,CString str_polength,CString str_podu,bool up );
	void drawquxian(const AcGePoint2d pt2d,const double d_xend,CString strbiao_start,CString strbiao_end,CString strRadio,bool up);
	void drawzhantai(const AcGePoint2d pt2d,const double d_xend,CString strStation,int iType,bool up);
	static AcDbObjectId AddRect2( const AcGePoint2d &startPoint, const AcGePoint2d &endPoint, double hight, double width ,AcDbDatabase *pDb = acdbHostApplicationServices()->workingDatabase() );
};
