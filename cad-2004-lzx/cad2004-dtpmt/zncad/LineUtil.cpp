...
AcDbObjectId CLineUtil::AddRect2( const AcGePoint2d &startPoint, const AcGePoint2d &endPoint, double hight, double width ,AcDbDatabase *pDb /*= acdbHostApplicationServices()->workingDatabase() */ )
{
	AcGePoint2d ptleftBottom(startPoint.x,(startPoint.y-hight));
	AcGePoint2d ptRightBottom(endPoint.x,(endPoint.y-hight));
	AcGePoint2d ptRightTop(endPoint.x,endPoint.y);
	AcGePoint2d ptLeftTOP(startPoint.x,startPoint.y);
	AcDbPolyline *poly =new AcDbPolyline(4);
	poly->addVertexAt(0,ptleftBottom,0,width,width);
	poly->addVertexAt(1,ptRightBottom,0,width,width);
	poly->addVertexAt(2,ptRightTop,0,width,width);
...
void CLineUtil::drawpodu( const AcGePoint2d pt2d,const double d_xend,CString strbiao,CString str_polength,CString str_podu,bool up )
{
	...
	idTextStyleChs = mysis.CreateTextStyle(_T("CHINESE"), _T("Times New Roman"), _T(""), 2.5, 0.6) ;
	AcGePoint2d pt2dstart;
	if(up==true)
		pt2dstart=pt2d;
	else
	{
	...
	if(up==true)
		...
		pt2dend.y-=15;
	AddPolyLine(pt2dstart,pt2dend,0.1);
	AcGePoint2d pt2dstart2=pt2dend;
	pt2dstart2.y=pt2dstart.y;
	AcGePoint2d pt2dend2=pt2dstart2;
	if(up==true)
		pt2dend2.y+=15;
	else
		pt2dend2.y-=15;
	AddPolyLine(pt2dstart2,pt2dend2,0.1);
	AcGePoint3d pt_3d_insert_text(pt2dstart2.x-2,pt2d.y+1,0);
	mysis.AddText(strbiao, pt_3d_insert_text, pt_3d_insert_text, AcDb::kTextLeft, AcDb::kTextVertMid, 3, 0.55, PI/2) ;	
	AcGePoint3d pt_3d_insertpodu((pt2dstart.x+pt2dend.x)/2,((pt2dstart.y+pt2dend.y)/2+4.5),0);
	str_podu=_T("坡度")+str_podu;
	mysis.AddText(str_podu, pt_3d_insertpodu, pt_3d_insertpodu, AcDb::kTextCenter, AcDb::kTextVertMid, 2.5, 1, 0) ;	
	AcGePoint3d pt_3d_insertpoleng((pt2dstart.x+pt2dend.x)/2,((pt2dstart.y+pt2dend.y)/2-4.5),0);
	str_polength=_T("坡长")+str_polength;
	mysis.AddText(str_polength, pt_3d_insertpoleng, pt_3d_insertpoleng, AcDb::kTextCenter, AcDb::kTextVertMid, 2.5, 1, 0) ;	
}

void CLineUtil::drawquxian( const AcGePoint2d pt2d,const double d_xend,CString strbiao_start,CString strbiao_end,CString strRadio,bool up )
{
	AcDbObjectId  idTextStyleTitle, idTextStyleContent, idTextStyleChs;
	idTextStyleTitle = mysis.CreateTextStyleByFileName(_T("zhffontTitle"), _T("txt.shx"), _T("hztxt2.shx"), 4, 0.7) ;	
	idTextStyleContent = mysis.CreateTextStyleByFileName(_T("zhffontContent"), _T("txt.shx"), _T("hztxt2.shx"), 3, 0.7) ;
	...
	AcGePoint2d pt2dend1=pt2d;
	pt2dend1.y+=15;
	AddPolyLine(pt2dstart,pt2dend1,0.1);
	AcGePoint3d pt_3d_insertlibiaostart(pt2dstart.x-2,pt2dstart.y+1,0);
	mysis.AddText(strbiao_start,pt_3d_insertlibiaostart,pt_3d_insertlibiaostart,AcDb::kTextLeft, AcDb::kTextVertMid, 3, 0.55, PI/2);
	AcGePoint3d pt_3d_insertlibiaoend(d_xend+2,pt2dstart.y+1,0);
	mysis.AddText(strbiao_end,pt_3d_insertlibiaoend,pt_3d_insertlibiaoend,AcDb::kTextLeft, AcDb::kTextVertMid, 3, 0.55, PI/2);
	pt2dstart.x=d_xend;
	pt2dend1.x=d_xend;
	AddPolyLine(pt2dstart,pt2dend1,0.1);
	...
	AcGePoint2d pt2d3;
	AcGePoint2d pt2d4;
	AcGePoint3d pt_3d_insertradio;
	if(up==true)
	{
		pt2d1=pt2d;
		pt2d1.y+=7.5;
		pt2dArray.append(pt2d1);
		pt2d2=pt2d1;
		pt2d2.x+=2.4;
		pt2d2.y+=3;
		pt2dArray.append(pt2d2);
		pt2d3=pt2d2;
		pt2d3.x=d_xend-2.4;
		pt2dArray.append(pt2d3);
		pt2d4=pt2d3;
		pt2d4.x+=2.4;
		pt2d4.y-=3;
		pt2dArray.append(pt2d4);
		pt_3d_insertradio.x=(pt2d.x+pt2d4.x)/2;
		pt_3d_insertradio.y=(pt2d1.y-1.5);
		pt_3d_insertradio.z=0;
	}
	else
	{
		pt2d1=pt2d;
	...
	CPolylineUtil::Add(pt2dArray,0.1);
	mysis.AddText(strRadio,pt_3d_insertradio,pt_3d_insertradio,AcDb::kTextCenter, AcDb::kTextVertMid, 2.5, 1, 0,7,idTextStyleChs);
   
}

void CLineUtil::drawzhantai( const AcGePoint2d pt2d,const double d_xend,CString strStation,int iType,bool up )
 {
// 	AcGePoint2d pt2dStart=pt2d;
// 	AcGePoint2d pt2dEnd=pt2dStart;
// 	double d_hight=0;
