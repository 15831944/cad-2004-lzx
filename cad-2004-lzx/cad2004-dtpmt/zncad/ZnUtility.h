...
public:
	...
	static	int DT_FindConBox(const AcGePoint3d& pt_3d_base, CLongArray& nArrayHandleFind, double dOffset=0.1);
	static int DT_GetCableParam(double& dWanquRatio, int& iZuoTou, double& dGuoGuiPer, double& dRFMPer, double& dAZPer, double& dJXPer,double& dSXJiaPer, int& iDistJX);
	// 返回对象的类类型
