...
void CZnSxt::GetPathGuiDao4( long nHandleLineStart,long nHandleLineEnd,vector<CString>&sVectorPath,vector<CString>&sDaocha, CStringArray &DaochaNum,CStringArray &GuDao,CLongArray&nArrayHandleLine,bool bForword/*=true*/ )
{
	...
	if(nHandleLineEnd!=0&&nHandleLineEnd==nHandleLineStart)
	{
		...
		{	
			if(nArrayDaoChaNum[k].GetString()!=_T(""))
			{
...
void CZnSxt::GetPathGuiDaoAndXHJ( long nHandleLineStart,long nHandleLineEnd,long LastDaocha,vector<CString>&sVectorPath,vector<CString>&sDaocha, vector<CString>& sXHJ,CStringArray &GuDao,CLongArray&nArrayHandleLine,bool bForword/*=true*/ )
{
	...
	if(nHandleLineEnd!=0&&nHandleLineEnd==nHandleLineStart)
	{
		...
		{
			CString name_xhj;
			GetXhjEntName(nArrayPassXHJ[k],name_xhj);
...
void CZnSxt::GetLieChePathGuiDaoAndXHJ( long nHandleStartXHJ,long nHandleEndXHJ,long nHandleLineStart,long nHandleLineEnd,long LastDaocha,vector<CString>&sVectorPath,vector<CString>&sDaocha, vector<CString>& sXHJ,CStringArray &GuDao,CLongArray&nArrayHandleLine,bool bForword/*=true*/ )
{
	...
	if(nHandleLineEnd!=0&&nHandleLineEnd==nHandleLineStart)
	{
		...
		oper_excel.WriteDatatoExcel(1,num,13,tempXHJ);
			...
			CString AnniuEnd;
			CString AnniuStart;
			GetXhjEntName(nHandleStartXHJ,AnniuStart);
			oper_excel.WriteDatatoExcel(1,num,9,AnniuStart);
			AnniuStart+=_T("LA");
			GetXhjEntName(nHandleEndXHJ,AnniuEnd);
			AnniuEnd+=_T("LA");
			CString JinLuAnniu=AnniuStart+_T("    ")+AnniuEnd;
...
void CZnSxt::GetJieChePathGuiDaoAndXHJ( CString& Start_Gd_name,long nHandleStartXHJ,long nHandleEndXHJ,long& nHandleLineFirst,long nHandleLineStart,long nHandleLineEnd,long LastDaocha,vector<CString>&sVectorPath,vector<CString>&sDaocha, vector<CString>& sXHJ,CStringArray &GuDao,CLongArray&nArrayHandleLine,vector<CString> &vec_strGuiDaoCX,int &iJL_Cnt,bool bForword/*=true*/ )
{
	...
	if(nHandleLineEnd!=0&&nHandleLineEnd==nHandleLineStart)
	{
		...
		bool btoward=bForword;
		//获取终止轨道信号机
// 	    int i_line_pos=FindGuiDaoHandleFromVector(nHandleLineEnd);
//         if(i_line_pos!=-1)
// 		{
// 			//vector<long> nVecPassXHJ;
// 			if(btoward==false)
// 			{
// 				CBlkUtility::CopyDataLongArrayFromVector(nArrayPassXHJ,vec_GuiDaoInfo[i_line_pos].m_vecHandleXHJLeft);
// 			}
// 			else
// 			{
// 				CBlkUtility::CopyDataLongArrayFromVector(nArrayPassXHJ,vec_GuiDaoInfo[i_line_pos].m_vecHandleXHJRight);
// 
// 			}
// 		}
// 	
...
// 			}
		...
		for(iCntTemp=0;iCntTemp<nArrayHandleTem.GetCount()-1;iCntTemp++)
		{
			...
			if(my_sis.GetPlineWidthAt(nHandle,0,d_width1,d_width2)==Acad::eOk)
			{
				...
				break;
			long nHandleNext=0;
			//可以优化，等同于FindNextLineByJYJ
			if(this->FindNextLineByJYJ(nHandle,nHandleNext,bForword)==TRUE)
			{
...
void CZnSxt::GetFaChePathGuiDaoAndXHJ( CString& Start_Gd_name,long nHandleStartXHJ,long nHandleEndXHJ,long& nHandleLineFirst,long nHandleLineStart,long nHandleLineEnd,long LastDaocha,vector<CString>&sVectorPath,vector<CString>&sDaocha, vector<CString>& sXHJ,CStringArray &GuDao,CLongArray&nArrayHandleLine,vector<CString> &vec_strGuiDaoCX,int &iJL_Cnt,bool bForword/*=true*/ )
{
	...
	if(nHandleLineEnd!=0&&nHandleLineEnd==nHandleLineStart)
	{
		...
		for(int k=0;k<nArrayPassXHJ.GetCount();k++)
		{
			...
			GetXhjEntName(nArrayPassXHJ[k],name_xhj);
			CString EntName;
			GetXhjBlkNameByHandle(nArrayPassXHJ[k],EntName);
			//将发车信号机写作兼调车信号机
...
void CZnSxt::GetDiaoChePathGuiDaoAndXHJ( CString& StartXHJ,CString& EndXHJ,long& nHandleLineFirst,long nHandleLineStart,long nHandleLineEnd,long LastDaocha,vector<CString>&sVectorPath,vector<CString>&sDaocha, vector<CString>& sXHJ,CStringArray &GuDao,CLongArray&nArrayHandleLine,vector<CString>& vec_strGuiDaoCX,int &iJL_Cnt,bool bForword/*=true*/ )
{
	...
	if(nHandleLineEnd!=0&&nHandleLineEnd==nHandleLineStart)
	{
		...
		else
		{
			...
			//判断信号机方向
			int iToward=-1;
			if(this->GetXhjBlockToward(nHandleDC,iToward)==TRUE)
			{
				if(iToward==1)
				{
					//向右
		...
		for(int i=0;i<strArray_GXHJ.GetCount();i++)
		{
			...
			if(GetXhjBlockToward(nHandleDCTemp,fangxiang)==TRUE)
			{
		...
		long lastdc=0;
		CStringArray strArrayDXDC;
		CLongArray  nArrayHandleNext;
		FindNextDuiXiangDCByLine(nHandleLineEnd,lastdc,GuDao,nArrayHandleNext,strArrayDXDC,bForword);
		for(int i=0;i<strArrayDXDC.GetCount();i++)
		{
			vec_xhj_didui.push_back(strArrayDXDC.GetAt(i)+_T(","));
			//acutPrintf(_T("对向：%s"),strArrayDXDC.GetAt(i));
		}
		//进站信号机敌对
		...
		long lastdc3=0;
		CStringArray strArrayLCXHJ2;
		CLongArray nArrayHandleNext3;
		FindNextLieCheXHJByLine(nHandleLine_Start,lastdc3,GuDao,nArrayHandleNext3,strArrayLCXHJ2,!fangxiang);
		if(strArrayLCXHJ2.GetCount()!=0)
		{
			//只有进站信号机敌对信号存在,敌对列车信号才能存在
			long lastdc2=0;
			CStringArray strArrayLCXHJ;
			CLongArray nArrayHandleNext2;
			FindNextLieCheXHJByLine(nHandleLineEnd,lastdc2,GuDao,nArrayHandleNext2,strArrayLCXHJ,bForword);
			if(strArrayLCXHJ.GetCount()!=0&&strArrayLCXHJ2.GetCount()!=0)
			{
				for(int i=0;i<strArrayLCXHJ2.GetCount();i++)
				{
					vec_xhj_didui.push_back(strArrayLCXHJ2.GetAt(i)+_T("L,"));
					//acutPrintf(_T("同向敌对：%s"),strArrayLCXHJ2.GetAt(i));
				}
				for(int i=0;i<strArrayLCXHJ.GetCount();i++)
				{
					vec_xhj_didui.push_back(strArrayLCXHJ.GetAt(i)+_T("L，"));
					//acutPrintf(_T("反向敌对：%s"),strArrayLCXHJ.GetAt(i));
				}
			}
		}
		...
		CString str;
		str.Format(_T("GetDiaoChePathGuiDaoAndXHJ time:%d ms"),t2-t1);
		iJL_Cnt++;
		//acutPrintf(_T("\n %s"),str);
		return;
	}
	...
	if(nHandleLineEnd!=0&&nHandleLineEnd==nHandleLineStart)
	{
		...
		if(LastDaocha==0)
		{
			...
			for(int i=0;i<nArrayHandleXHJTemp.GetCount();i++)
			{
				CString xhjTemp;
				CString xhjEnt;
				GetXhjEntName(nArrayHandleXHJTemp.GetAt(i),xhjTemp);
		...
		else if((item=CBlkUtility::FindStrInArray(this->sSGuDaoName,temp))!=-1)
		{
			...
			tempGuidao.pop_back();
			//换成股道名称
			if(item>=this->str_GuDaoName.GetCount())
			{
				AfxMessageBox(_T("item 超过界限"));
				return ;
			}
			jinlu=_T("至")+this->str_GuDaoName[item];
			if(count>0)
			...
			if(this->GetXhjBlockToward(nHandleDC,iToward)==TRUE)
			{
				if(iToward==1)
				{
					...
					if(bForword==true)
					{
						...
						if(count>1)
						{
							...
							else
							{
								...
								if(xiangxhj!=0)
								{
									GetXhjEntName(xiangxhj,str_xxhj);
					...
					else
					{
						...
						CString xhjent;
						GetXhjBlkNameByHandle(nHandleDC,xhjent);
...
// 							AnniuEnd =str_array_xhj[count-1]+_T("A");
		...
		if(i_m!=-1)
		{
			if(i_m<0||i_m>=this->strArrayGuiDaoCX.GetCount())
			{
				AfxMessageBox(_T("i_m 超限"));
				return ;
			}
			vec_strGuiDaoCX.push_back(this->strArrayGuiDaoCX.GetAt(i_m));
		}
		...
		//这个函数对于左方向来说求得的对向调车信号机不全
		if(StartXHJ==_T("D3")&&EndXHJ==_T("D5"))
		{
			StartXHJ=StartXHJ;
		}
		FindNextDuiXiangDCByLine2(nHandleLineEnd,lastdc,GuDao,nArrayHandleNext,strArrayDXDC,strArrayDXDaocha,bForword);
		CLongArray nArrayHandleDXDC;
		for(int k=0;k<strArrayDXDC.GetCount();k++)
		{
			//vec_xhj_didui.push_back(strArrayDXDC.GetAt(i)+_T(","));
			int i_add=0;
			//acutPrintf(_T("对向：%s"),strArrayDXDC.GetAt(k));
			long nHandleDXDC=0;
			//注意这里需要去掉_T('D')
			CString tempXHJ=strArrayDXDC.GetAt(k);
			...
			if(iCnt_TXDC>1)
			{
				...
				for(int i=0;i<strArrayTXDC.GetCount();i++)
				{
					...
					if(iTJ_CntTem>=1)
					{
						...
						CString str_temp;
						if(k<0||k>=strArrayDXDaocha.GetCount())
						{
							AfxMessageBox(_T("strArrayDxDaocha访问超限，k=%d,cnt=%d"),k,strArrayDXDaocha.GetCount());
							return ;
						}
			            if(strArrayDXDaocha.GetAt(k)!=_T(""))
						{
							...
							vec_xhj_didui.push_back(str_temp+strArrayDXDC.GetAt(k));
						}
						...
						{
							vec_xhj_didui.push_back(vec_strTiaojianT[0]+strArrayDXDC.GetAt(k));
						}
						i_add=1;
      					break;
					}
				...
				if(i_add==0)
				{
					...
					vec_xhj_didui.push_back(strArrayDXDC.GetAt(k));
				}
		...
		//寻找敌对进站信号机
		FindNextLieCheXHJByLine(nHandleLine_StartNew,lastdc3,GuDao,nArrayHandleNext3,strArrayLCXHJ2,!bForword);
		if(nArrayHandleNext3.GetCount()!=strArrayLCXHJ2.GetCount())
		{
			AfxMessageBox(_T("strArrayLCXHJ2个数不相等"));
			return ;
		}
		if(strArrayLCXHJ2.GetCount()!=0)
		{
			...
			//寻找敌对出站信号机
			FindNextLieCheXHJByLine(nHandleLineEnd,lastdc2,GuDao,nArrayHandleNext2,strArrayLCXHJ,bForword);
			if(nArrayHandleNext2.GetCount()!=strArrayLCXHJ.GetCount())
			{
				AfxMessageBox(_T("strArrayLCXHJ个数不相等"));
				return ;
			}
			if(strArrayLCXHJ.GetCount()!=0)
			{
				for(int i=0;i<strArrayLCXHJ2.GetCount();i++)
				{
				     //是否存在条件
					...
					CString str_te=strArrayLCXHJ2.GetAt(i);
					CString entname;
					long nhandle=0;
					CString xhjname=strArrayLCXHJ2.GetAt(i);
					 xhjname.Remove(_T('L'));
					GetHandleByName(xhjname,nhandle);
					GetXhjBlkNameByHandle(nhandle,entname);
					if(entname.Left(6)==_T("XHJ_LJ"))
					{
						...
						/*	CString Gd_name;
							...
							if(bSXiaxing==true)
							{
								for(int k=0;k<this->nAarryHandleGuDaoS.GetCount();k++)
								{
									...
									if(i<0||i>=nArrayHandleNext3.GetCount())
									{
										AfxMessageBox(_T("nArrayHandleNext3越界 i=%d,Max=%d"),i,nArrayHandleNext3.GetCount());
										return ;
									}
									else 
							...
							{
								for(int k=0;k<this->nAarryHandleGuDaoX.GetCount();k++)
								{
									if(i<0||i>=nArrayHandleNext3.GetCount())
									{
										AfxMessageBox(_T("nArrayHandleNext3越界 i=%d,Max=%d"),i,nArrayHandleNext3.GetCount());
							...
							{
								CString str_temp;
								if(i<0||i>=strArrayLCXHJ2.GetCount())
								{
									AfxMessageBox(_T("nArrayLCXHJ2越界 i=%d,Max=%d"),i,strArrayLCXHJ2.GetCount());
									return ;
								}
								else 
								str_temp=_T("<>")+strArrayLCXHJ2.GetAt(i);
								//str_temp+=_T(">");
						...
						if(mm==0)
						{
								...
								vec_xhj_didui.push_back(strArrayLCXHJ2.GetAt(i));
						}	
					...
					else if(entname.Left(6)==_T("XHJ_LF"))
					{
						...
						if(bSXiaxing==true)
						{
							...
							{
								if(i<0||i>=nArrayHandleNext3.GetCount()||k>=nAarryHandleSJJC.GetCount())
								{
									CString tishi;
									tishi.Format(_T("nArrayHandleNext3越界 i=%d,Max1=%d,Max2=%d"),i,nArrayHandleNext3.GetCount(),nAarryHandleSJJC.GetCount());
									AfxMessageBox(tishi);
									return ;
								}
								else 
						...
						{
							for(int k=0;k<this->nAarryHandleXJJC.GetCount();k++)
							{
								if(i<0||i>=nArrayHandleNext3.GetCount()||k>=nAarryHandleXJJC.GetCount())
								{
									CString tishi;
									tishi.Format(_T("nArrayHandleNext3越界 i=%d,Max1=%d,Max2=%d"),i,nArrayHandleNext3.GetCount(),nAarryHandleXJJC.GetCount());
									AfxMessageBox(tishi);
									return ;
								}
								else 
				...
				for(int i_m=0;i_m<strArrayLCXHJ.GetCount();i_m++)
				{
					int kk=0;
					int temp=nArrayHandleNext2.GetCount();
					int temp2=nArrayHandleNext3.GetCount();
					...
					//m_mysys.GetXdata(nArrayHandleNext2.GetAt(i),_T("GUIDAONAME"),Gd_name);
 					CString str_te=strArrayLCXHJ.GetAt(i_m);
					CString entname;
					long nhandle=0;
					CString xhjname=strArrayLCXHJ.GetAt(i_m);
					xhjname.Remove(_T('L'));
					GetHandleByName(xhjname,nhandle);
					GetXhjBlkNameByHandle(nhandle,entname);
					if(entname.Left(6)==_T("XHJ_LF"))
					{
						...
						int iJL_CntTem=0;
						int iTJ_CntTem=0;
						//if(this->nAarryHandleSJJC.GetCount()!=this->nAarryHandleXJJC.GetCount())
						//{
						//	AfxMessageBox(_T("SJJC和XJJC不相等"));
						//	return ;
						//}
						if(bSXiaxing==true)
						{
							for(int k=0;k<this->nAarryHandleSJJC.GetCount();k++)
							{
								if(i_m<0||i_m>=nArrayHandleNext2.GetCount())
								{
									AfxMessageBox(_T("nArrayHandleNext2越界 i_m=%d,Max=%d"),i_m,nArrayHandleNext2.GetCount());
									return ;
								}
								else 
									this->IsAddConditionDiaoChePath(nArrayHandleNext2.GetAt(i_m),this->nAarryHandleSJJC.GetAt(k),nArrayHandleTem2,lastdcTemp,vec_sDaochaTem,GuDao,nArrayHandleLineTem2,vec_strTiaojianT,iJL_CntTem,iTJ_CntTem,true);
							}
						}
						else
						{
							for(int k=0;k<this->nAarryHandleXJJC.GetCount();k++)
							{
								...
								this->IsAddConditionDiaoChePath(nArrayHandleNext2.GetAt(i_m),this->nAarryHandleXJJC.GetAt(k),nArrayHandleTem2,lastdcTemp,vec_sDaochaTem,GuDao,nArrayHandleLineTem2,vec_strTiaojianT,iJL_CntTem,iTJ_CntTem,false);
							}
						}
						//this->IsAddConditionDiaoChePath(nArrayHandleNext2.GetAt(i),this->nAarryHandleSJJC.GetAt(k),nArrayHandleTem2,lastdcTemp,vec_sDaochaTem,GuDao,nArrayHandleLineTem2,vec_strTiaojianT,iJL_CntTem,iTJ_CntTem,true);
						if(iTJ_CntTem>=1)
						{		
							kk=1;
							CString str_temp;
							if(i_m<0||i_m>=strArrayLCXHJ.GetCount())
							{
								AfxMessageBox(_T("strArrayLCXHJ越界 i_m=%d,Max=%d"),i_m,strArrayLCXHJ.GetCount());
								return ;
							}
							else 
							str_temp=_T("<>")+strArrayLCXHJ.GetAt(i_m);
							//str_temp+=_T(">");
							vec_xhj_didui.push_back(str_temp);
						//	break;
						}
					//}
						if(kk==0)
						{
							...
							vec_xhj_didui.push_back(strArrayLCXHJ.GetAt(i_m));
						}
					}
					else if(entname.Left(6)==_T("XHJ_LJ"))
					{
						...
						if(bSXiaxing==true)
						{
							for(int k=0;k<this->nAarryHandleGuDaoS.GetCount();k++)
							{
								...
								this->IsAddConditionDiaoChePath(nArrayHandleNext2.GetAt(i_m),this->nAarryHandleGuDaoS.GetAt(k),nArrayHandleTem2,lastdcTemp,vec_sDaochaTem,GuDao,nArrayHandleLineTem2,vec_strTiaojianT,iJL_CntTem,iTJ_CntTem,false);
							}
						}
						else
						{
							for(int k=0;k<this->nAarryHandleGuDaoX.GetCount();k++)
							{
								...
								this->IsAddConditionDiaoChePath(nArrayHandleNext2.GetAt(i_m),this->nAarryHandleGuDaoX.GetAt(k),nArrayHandleTem2,lastdcTemp,vec_sDaochaTem,GuDao,nArrayHandleLineTem2,vec_strTiaojianT,iJL_CntTem,iTJ_CntTem,true);
							}
						}
						...
						if(iTJ_CntTem>=1)
						{
							...
							CString str_temp=_T("<>")+strArrayLCXHJ.GetAt(i_m);
							//str_temp+=_T(">");
							vec_xhj_didui.push_back(str_temp);
							kk=1;
							//break;
		...
		oper_excel.WriteDatatoExcel(1,num,9,Startlcxhj);
		//2016-03-02 修改
		AnniuStart=StartXHJ+_T("A");
		//AnniuStart=Startlcxhj+_T("A");
		//AnniuEnd=temp+_T("A");
		CString JinLuAnniu=AnniuStart+_T("    ")+AnniuEnd;
		oper_excel.WriteDatatoExcel(1,num,7,JinLuAnniu);
		oper_excel.WriteDatatoExcel(1,num,10,_T("B"));
	...
	else
	{
		...
		CStringArray PassXHJ;
		long t1=GetTickCount();
		long icnt1=m_RecordSet.i_count;
		...
		long t2=GetTickCount();
		long icnt2=m_RecordSet.i_count;
		//acutPrintf(_T("\n DiaocheFindNextLineAndXHJ2=%d ms \t %d 次"),t2-t1,icnt2-icnt1);
		int count =0;
		count =nArrayHandleNext.GetCount();
		if(count!=0)
		{
...
//		CStringArray PassXHJ;
		...
		int count =0;
		count =nArrayHandleNext.GetCount();
		if(nArrayHandleThisDC.GetCount()!=count)
		{
			AfxMessageBox(_T("ArrayHandleThisDC和nArrayHandleNext个数不等"));
			return ;
		}
		if(count!=0)
		{
				...
				else
				{
					...
					if(nHandleDC!=0)
					{
						...
						if(bForword==true&&IsStart>0)
						{
							...
							{
								// Daocha=_T("(")+DCname+_T(")");
								GetDaoChaEntName(nHandleDaiDong_nMin,DaiDong_MinName);
								GetDaoChaEntName(nHandleDaiDong_nMax,DaiDong_MaxName);
					...
					if(nHandleDC!=0)
					{
						...
						if(bForword==true&&IsStart>0)
						{
							...
							this->GetGuiDaoDuanXHJ(bForword,nHandle_dw,Zhi_XHJ,Xiang_XHJ);
								...
								if((GetXhjBlockPosition_X(Zhi_XHJ,pt_zxhj_x)==TRUE)&&(GetXhjBlockPosition_X(nHandleDC,pt_DC_x)==TRUE))
								{
								...
								if((GetXhjBlockPosition_X(Zhi_XHJ,pt_xxhj_x)==TRUE)&&(GetXhjBlockPosition_X(nHandleDC,pt_DC_x)==TRUE))
								{
						...
						else if(bForword==false&&IsStart==0)
						{
							...
							long Zhi_XHJ=0;
							long Xiang_XHJ=0;
							if((GetXhjBlockPosition_X(Zhi_XHJ,pt_zxhj_x)==TRUE)&&(GetDaochaBlockPosition_X(nHandleDC,pt_DC_x)==TRUE))
							{
								if(pt_zxhj_x<pt_DC_x)
									nArrayZhiXHJ.Add(Zhi_XHJ);
							}
							...
							if((GetXhjBlockPosition_X(Zhi_XHJ,pt_xxhj_x)==TRUE)&&(GetDaochaBlockPosition_X(nHandleDC,pt_DC_x)==TRUE))
							{
								if(pt_xxhj_x<pt_DC_x)
									nArrayXiangXHJ.Add(Xiang_XHJ);
							}
						...
						if(nLineType==1)
						{
							...
							if(nHandleDC2!=0)
							{
								...
								if(nHandle_dw2!=0)
								{
									...
									long Xiang_XHJ=0;
									this->GetGuiDaoDuanXHJ(bForword,nHandle_dw2,Zhi_XHJ,Xiang_XHJ);
									if((GetXhjBlockPosition_X(Zhi_XHJ,pt_zxhj_x)==TRUE)&&(GetDaochaBlockPosition_X(nHandleThisDC,pt_DC_x)==TRUE))
									{
										if(pt_zxhj_x>pt_DC_x)
											nArrayZhiXHJ.Add(Zhi_XHJ);
									}
									...
									if((GetXhjBlockPosition_X(Zhi_XHJ,pt_xxhj_x)==TRUE)&&(GetDaochaBlockPosition_X(nHandleThisDC,pt_DC_x)==TRUE))
									{
										if(pt_xxhj_x>pt_DC_x)
											nArrayXiangXHJ.Add(Xiang_XHJ);
									}
...
void CZnSxt::DiaoCheFindNextLineAndXHJ2( long nHandleLineStart,long nHandleLineEnd,CStringArray &GuoDaoCha,CStringArray &Gudao,long LastDaocha,CLongArray& nArrayHandleThisDaocha,CStringArray& GuoXHJ,CLongArray& nArrayHandleLine,CStringArray&strCXGuiDaoName,bool bForword/*=true*/ )
{
	try
	{
		...
		int nLineType = 0 ;
		//m_RecordSet.GetRecordValue(strSql, _T("sxt_line_type"), nLineType) ;
		this->FindLineType(nHandleLineStart,nLineType);
		//表示为尽头线
		if (nLineType==2) 
		{
			CString name;
		...
		long nHandleLeftJyj = 0, nHandleRightJyj = 0 ;
		long nHandleNextLine = 0 ;
		this->FindNextLineByJYJ(nHandleLineStart,nHandleNextLine,bForword);
		if(iNumDC<1)
		{
			//轨道不包含道岔时，后继只有一个
			if(nHandleLineStart==nHandleLineEnd)
			...
			{//不为渡线时
				...
				else
				{
					...
					long nHandleDC=0;
					//m_RecordSet.GetRecordValue(strSql, _T("sxt_ent_handle"), nHandleDC);
					this->FindDaocha_Byfwx(nHandleLineStart,nHandleDC);
					if(nHandleDC!=0)
					{
						//查询道岔所在的线，该线为后继
						strSql.Format(_T("select * from sxt_ent where sxt_ent_handle=%d"), nHandleDC) ;
						...
						CString DCname;
						CString str_Daidong;
						int i_pos=FindDaoChaHandleFromVector(nHandleDC);
						if(i_pos!=-1)
						{
							nHandle_dw=vec_DaochaInfo[i_pos].m_nHandleLine_dw;
							IsStart=vec_DaochaInfo[i_pos].m_nHandleLine_fw_vertex;
							DCname=vec_DaochaInfo[i_pos].m_strDcName;
							str_Daidong=vec_DaochaInfo[i_pos].m_strDaidong;
						}
						if(bForword==true&&IsStart>0)
						{
							//定位所在线为后继
...
// 				strSql.Format(_T("select * from sxt_ent where sxt_ent_handle=%d"), LastDaocha) ;
// 				m_RecordSet.GetRecordValue(strSql, _T("sxt_ent_x"),d_x) ;
				this->GetDaochaBlockPosition_X(LastDaocha,d_x);
			}
			CString DaochaD2;
			for(int k=index_lastDaocha+1;k<iNumDC;k++)
			{//道岔反位时后继,需要前面访问的道岔
				...
				if(i_pos!=-1)
				{
					DCname1=vec_DaochaInfo[i_pos].m_strDcName;
					nHanderDC_another=vec_DaochaInfo[i_pos].m_nHandle_Another_DC;
				    nMinDc=vec_DaochaInfo[i_pos].m_nHandleDC_Min;
					nMaxDc=vec_DaochaInfo[i_pos].m_nHandleDC_Max;
					nHandle_Anothe_Min=vec_DaochaInfo[i_pos].m_nHandle_Jiaocha_DCMin;
					nHandle_Anothe_Max=vec_DaochaInfo[i_pos].m_nHandle_Jiaocha_DCMax;
					DCname_Min=vec_DaochaInfo[i_pos].m_strDC_Min;
					DCname_Max=vec_DaochaInfo[i_pos].m_strDC_Max;
					DC_Another_Min=vec_DaochaInfo[i_pos].m_str_Jiaocha_DCMin;
					DC_Another_Max=vec_DaochaInfo[i_pos].m_str_Jiaocha_DCMax;
				}
				if(nHanderDC_another==0)
				{
					if(k==iNumDC-1)
					{
						DaochaD2=DaochaD2+DCname1;
					}
					else
					{
						DaochaD2=DaochaD2+DCname1+_T(",");
					}
				}
				else
				{
					if(k==iNumDC-1)
					{
						if(nHandle_Anothe_Min!=0&&nHandle_Anothe_Max!=0)
						{
							DaochaD2 =DaochaD2+DCname_Min+_T("/")+DCname_Max+_T(",")+DC_Another_Min+_T("/")+DC_Another_Max;
						}
						else
						{
							DaochaD2 =DaochaD2+DCname_Min+_T("/")+DCname_Max+_T(",");
						}
					}
					else
					{
						if(nHandle_Anothe_Min!=0&&nHandle_Anothe_Max!=0)
						{
							DaochaD2 =DaochaD2+DCname_Min+_T("/")+DCname_Max+_T(",")+DC_Another_Min+_T("/")+DC_Another_Max+_T(",");
						}
						else
						{
							DaochaD2 =DaochaD2+DCname_Min+_T("/")+DCname_Max+_T(",");
			...
			if(nHandleNextLine!=0)
			{
				...
				for(int k=0;k<nPassXHJ.GetCount();k++)
				{
					...
					GetXhjEntName(nPassXHJ[k],name);
					CString EntName;
					GetXhjBlkNameByHandle(nPassXHJ[k],EntName);
					if(EntName.Left(6)==_T("XHJ_LF"))
					{
						name+=_T("D");
					}
			...
			for(int k=index_lastDaocha+1;k<iNumDC;k++)
			{
				...
				if(i_pos!=-1)
				{
					DCname1=vec_DaochaInfo[i_pos].m_strDcName;
					nHanderDC_another=vec_DaochaInfo[i_pos].m_nHandle_Another_DC;
					nHandle_fw=vec_DaochaInfo[i_pos].m_nHandleLine_fw;
					nHandle_dw=vec_DaochaInfo[i_pos].m_nHandleLine_dw;
					nMinDc=vec_DaochaInfo[i_pos].m_nHandleDC_Min;
					nMaxDc=vec_DaochaInfo[i_pos].m_nHandleDC_Max;
					nHandle_Anothe_Min=vec_DaochaInfo[i_pos].m_nHandle_Jiaocha_DCMin;
				...
				if(IsStart==0&&bForword==true)
				{
					...
					if(nHandle_fw!=0)
					{
						...
						//strSql.Format(_T("select * from sxt_line where sxt_line_handle=%d"), nHandle_fw) ;
						int nLineType = 0 ;
						this->FindLineType(nHandle_fw,nLineType);
						//m_RecordSet.GetRecordValue(strSql, _T("sxt_line_type"), nLineType) ;
						if(nLineType==1)
						{
							//为渡线时，求反位线的后继
							long nHandleNextLine2= 0 ;
							this->FindNextLineByJYJ(nHandle_fw,nHandleNextLine2,bForword);
							//再找该反位线所在道岔的定位线
							//strSql.Format(_T("select * from sxt_ent where sxt_ent_fwline_handle=%d"), nHandleNextLine2) ;
							//CString strFieldName = (bForword?_T("sxt_ent_conline_right"):_T("sxt_ent_conline_left")) ;
							//反位线对应的道岔
							long nHandleDC2=0;
							//m_RecordSet.GetRecordValue(strSql, _T("sxt_ent_handle"), nHandleDC2);
							this->FindDaocha_Byfwx(nHandleNextLine2,nHandleDC2);
							if(nHandleDC2!=0)
							{
								//查询道岔所在的线，该线为后继
								//strSql.Format(_T("select * from sxt_ent where sxt_ent_handle=%d"), nHandleDC2) ;
								...
								//m_RecordSet.GetRecordValue(strSql, _T("sxt_ent_online_handle"), nHandle_dw2) ;
								//可以优化
								int i_pos_2=FindDaoChaHandleFromVector(nHandleDC2);
								if(i_pos_2!=-1)
								{
									nHandle_dw2=vec_DaochaInfo[i_pos_2].m_nHandleLine_dw;
								}
								//nHandle_dw2即为渡线的后继
								if(nHandle_dw2!=0)
								{
								...
								if(nHandle_Anothe_Min!=0&&nHandle_Anothe_Max!=0)
								{
									CString Daocha=DaochaD+_T("(")+DCname_Min+_T("/");
									Daocha=Daocha+DCname_Max+_T(")");
									Daocha=Daocha+_T(",")+_T("[")+DC_Another_Min+_T("/")+DC_Another_Max+_T("]");
									if(str_Daidong!=_T(""))
									{
										Daocha=Daocha+_T(",")+str_Daidong;
				...
				else
				{
					if(k==iNumDC-1)
					{
						if(nHandle_Anothe_Max!=0&&nHandle_Anothe_Min!=0)
						{
							DaochaD =DaochaD+DCname_Min+_T("/")+DCname_Max+_T(",")+DC_Another_Min+_T("/")+DC_Another_Max;
							//acutPrintf(_T("\n遇到交叉渡线3"));
						}
	...
	{	
		AfxMessageBox(_T("未知的错误!in DiaoCheFindNextLineAndXHJ2"));
	}	
...
void CZnSxt::DiaoCheFindNextLineAndDaocha( long nHandleLineStart,long nHandleLineEnd,CStringArray &Gudao,long LastDaocha,CLongArray& nArrayHandleThisDaocha,CLongArray& nArrayHandleLine,bool bForword/*=true*/ )
{
	try
	{
		...
		int iNumDC = nArrayHandleDC.GetCount() ;//当前轨道电路包含几个道岔
		long nHandleNextLine = 0 ;
		FindNextLineByJYJ(nHandleLineStart,nHandleNextLine,bForword);
		if(iNumDC<1)
		{
			//轨道不包含道岔时，后继只有一个
			if(nHandleLineStart==nHandleLineEnd)
				...
				if(IsStart==0&&bForword==true)
				{
					...
					if(nHandle_fw!=0)
					{
						...
						if(nLineType==1)
						{
							...
							long nHandleNextLine2 = 0 ;
							FindNextLineByJYJ(nHandle_fw,nHandleNextLine2,bForword);
							//反位线对应的道岔
							//再找该反位线所在道岔的定位线
							//strSql.Format(_T("select * from sxt_ent where sxt_ent_fwline_handle=%d"), nHandleNextLine2) ;
							long nHandleDC2=0;
...
void CZnSxt::LieCheFindNextLineAndXHJ( long nHandleLineStart,long nHandleLineEnd,CStringArray &GuoDaoCha,CStringArray &Gudao,long LastDaocha,CLongArray& nArrayHandleThisDaocha,CStringArray& GuoXHJ,CLongArray& nArrayHandleLine,CStringArray &strCXGuiDaoName,bool bForword/*=true*/ )
{
	try
	{
		...
		this->FindLineType(nHandleLineStart,nLineType);
		//m_RecordSet.GetRecordValue(strSql, _T("sxt_line_type"), nLineType) ;
		//nLineType=vec_GuiDaoInfo[i_pos_line].m_nLineType;
		//表示为尽头线
		if (nLineType==2) 
		{
			return ;
...
int CZnSxt::GetAll_LJ_XHJ( CLongArray& nArrayHandleDc )
{
	...
	long t2=GetTickCount();
	CString str;
	str.Format(_T("GetAll_LJ_XHJ:%d ms"),t2-t1);
	//acutPrintf(_T("\n%s"),str);
	return iRet ;
...
BOOL CZnSxt::GetXhjBlockToward( long nHandleXHJ ,int &iToward)
{
	if(nHandleXHJ==0)
	{
		return FALSE;
	}
	int i_pos=this->FindXhjHandleFromVector(nHandleXHJ);
	if(i_pos!=-1)
	{
		iToward=this->vec_XhjInfo[i_pos].m_iToward;
		return TRUE;
	}
	return FALSE;
...
BOOL CZnSxt::SelectOpenFile(CString &strFilePath)
{
	try
	{
		CString strFilter = _T("(文件(*.xls)|*.xls|");
		CFileDialog fileDlg(TRUE, NULL, NULL, OFN_HIDEREADONLY, strFilter,NULL);
		//CSpecialDialog SpeDlg(TRUE,_T("xls"),NULL,OFN_OVERWRITEPROMPT|OFN_HIDEREADONLY,_T("Worksheet Files (*.xls)|*.xls||"),NULL);
		fileDlg.m_ofn.lpstrTitle = _T("选择xls文件");
		TCHAR szBuf[256];
		memset(szBuf, 0, sizeof(szBuf));
		GetCurrentDirectory(sizeof(szBuf), szBuf);
		fileDlg.m_ofn.lpstrInitialDir = szBuf;
...
void CZnSxt::LsbTest2()
{
	...
	//获取所有接车信号机==GetSomeBlocks(_T("XHJ_LJ"),nArrayHandle_LJ_XHJ);
...
// 		//strSql.Format(_T("select * from sxt_ent where sxt_ent_handle=%d"), nHandleLine) ;
// 		//m_RecordSet.GetRecordValue(strSql, _T("sxt_ent_name"),entname) ;
// 		this->GetXhjEntName(nHandleLine,entname);
// 		acutPrintf(_T("\n 出站信号机名称为%s"),entname.GetString());
// 		//查询进站信号机右侧直线
// 		strSql.Format(_T("select * from sxt_ent where sxt_ent_handle=%d"), nHandleLine);
// 		//获取信号机右侧轨道,即为股道,如果没有下行咽喉出站信号机的话
...
// 				gu.Remove(_T('X'));
// 				gu.Append(_T("G"));
// 				if(gu!=_T("")&&CBlkUtility::FindStrInArray(str_GuDaoName,gu)==-1)
...
// 			//这里可以优化
// 			nHandleLinetemp2=0;
// 			m_RecordSet.GetRecordValue(strSql, _T("sxt_ent_conline_left"),nHandleLinetemp2) ;
// 			//nAarryHandleGuDaoX.Add(nHandleLinetemp2);
// 			//打印股道名称
// 			if(nHandleLinetemp2!=0)
// 			{
...
// 	}
	...
	CBlkUtility::SortStringArray_CZXHJHandle(this->str_GuDaoName,sXGuDaoName,nAarryHandleGuDaoX,sSGuDaoName,nAarryHandleGuDaoS);
	//应该统一按照股道名称来排序；
	for(int i=0;i<str_GuDaoName.GetCount();i++)
	{
		acutPrintf(_T("\n股道名称为：%s"),str_GuDaoName.GetAt(i).GetString());
	}
	...
	// 		输出所有下行接车进路
		acutPrintf(_T("\n开始输出下行接车进路\n"));
		CString strBarMsg ;
		int iRsCount=nAarryHandleXJC.GetCount();
		strBarMsg.Format(_T("下行接车进路:%d进路"), iRsCount) ;
		...
		for(int i=0;i<nAarryHandleXJC.GetCount();i++)
		{
			...
			for(int m=0;m<nAarryHandleSJJC.GetCount();m++)
			{
				...
				GetXhjEntName(nxhj_end,str_fx);
				m_mysys.GetXdata(nHandleStart,_T("GUIDAONAME"),str_GdStartname);
				if(str_fx.Right(1)==_T("N")||str_fx.Right(1)==_T("F"))
				{	
			...
			for(int k=0;k<nAarryHandleGuDaoX.GetCount();k++)
			{
					...
					GetXhjEntName(nHandleStartXHJ,str_fx);
					long t32=GetTickCount();
					CString str32;
					str32.Format(_T("GetXhjEntName:%d ms"),t32-t31);
					acutPrintf(_T("\n%s"),str32);
					...
					t32=GetTickCount();
					str32.Format(_T("GetJieChePathGuiDaoAndXHJ:%d ms"),t32-t31);
					acutPrintf(_T("\n%s"),str32);
					//写确定运行方向道岔
					if(iJL_Cnt_temp>=2)
					{
			...
			for(int k=0;k<nAarryHandleGuDaoX.GetCount();k++)
			{
				...
				long t32=GetTickCount();
				CString str32;
				str32.Format(_T("GetFaChePathGuiDaoAndXHJ:%d ms"),t32-t31);
				acutPrintf(_T("\n%s"),str32);
				//CString str32;
				if(iJL_Cnt_temp>=2)
				{
		...
		for(int i=0;i<iNum_DC;i++)
		{
			...
			bool bforward=true;
			int iTo=1;
			GetXhjBlockToward(nHandleDCXHJ,iTo);
			if(iTo==1)
			{
				fangxiang=_T("右");
			}
			...
			//long LastDaocha=0;
			GetXhjEntName(nHandleDCXHJ,name);
		...
		long t32=GetTickCount();
		CString str32;
		str32.Format(_T("SortArray time:%d ms"),t32-t31);
		acutPrintf(_T("\n %s"),str32);
		long nHandleStartLine=0;
		bool bforward=true;
		//分上下行咽喉
		...
		//	acedSetStatusBarProgressMeter()
		 iRsCount=strArrayOdd.GetCount();
		strBarMsg.Format(_T("下行咽喉调车:%d进路"), iRsCount) ;
		acedSetStatusBarProgressMeter(strBarMsg, 0, iRsCount) ;
		for(int i=0;i<strArrayOdd.GetCount();i++)
		{
			CStringArray strNextDC;
			...
			long t32=GetTickCount();
			str32.Format(_T("GetNextDCXHJByDCXHJ:%d ms"),t32-t31);
			acutPrintf(_T("\n%s"),str32);
			CBlkUtility::SortStringArray_DCZhongduanxhj(strNextDC,nArrayHandleNext);
			//这里需要对调车终端进行排序
			CString xhj;
			GetXhjEntName(nHandleDCXHJ,xhj);
			//acutPrintf(_T("\n调车始端：%s"),xhj);
			//acutPrintf("\n调车终端：\n");
			...
			{	
				...
				//acutPrintf(_T("\t%s-->%s--%s"),xhj,strNextDC.GetAt(k),strGuiname);
			}
		 ...
		 if(nHandleStartLine!=0)
		 {
			 //输出调车进路
			 CString strBarMsg3 ;
			 int iRsCount3=nArrayHandleNext.GetCount();
			strBarMsg3.Format(_T("%s:%d 条"), temp,iRsCount3) ;
			acedSetStatusBarProgressMeter(strBarMsg3, 0, iRsCount3) ;
			 for(int i=0;i<nArrayHandleNext.GetCount();i++)
			 {
			    ...
			    acedSetStatusBarProgressMeterPos(i) ;	
				 ...
				 GetXhjEntName(nHandleDCXHJ,str_xhj_start);
				...
				long icnt2=m_RecordSet.i_count;				
				 long t2=GetTickCount();
				 acutPrintf(_T("\n GetDiaochePathGuiDaoAndXHJ2=%d ms \t%d 次"),t2-t1,icnt2-icnt1);
		...
		for(int i=0;i<nAarryHandleGuDaoS.GetCount();i++)
		{
			...
			CString strname;
			GetXhjEntName(nArrayHandleXHJ[0],strname);
		...
		CBlkUtility::SortStringArray_CZXHJHandle(str_name_ArrayXXHJ,nArrayXXHJ);
		iRsCount =nArrayXXHJ.GetCount();
		CString strBarMsg2 ;
		strBarMsg2.Format(_T("下行咽喉出站兼调车:%d进路"), iRsCount) ;
		acedSetStatusBarProgressMeter(strBarMsg2, 0, iRsCount) ;
...
// 			GetEntName(nHandleDCXHJ,xhj);
			...
			{
				//输出调车进路
				iRsCount =nArrayHandleNext.GetCount();
				CString strBarMsg4 ;
				strBarMsg4.Format(_T("%s:%d条"), xhj,iRsCount);
				acedSetStatusBarProgressMeter(strBarMsg4, 0, iRsCount) ;
				for(int i=0;i<nArrayHandleNext.GetCount();i++)
				{
					long nHandleEndLine=nArrayHandleNext.GetAt(i);
					long LastDaocha=0;
		...
		//输出下行发车兼调车进路
		//输出所有上行接车进路
		int i_row_shangxinlieche =this->num;
		acutPrintf(_T("\n开始输出上行接车进路\n"));
		//CString strBarMsg ;
		iRsCount=nAarryHandleSJC.GetCount();
		strBarMsg.Format(_T("上行接车进路:%d进路"), iRsCount) ;
		acedSetStatusBarProgressMeter(strBarMsg, 0, iRsCount) ;
		for(int i=0;i<nAarryHandleSJC.GetCount();i++)
		{
	  	...
	  	//列车进路结束
		acedRestoreStatusBar();
		int i_excel_row_lieche2=this->num;
		oper_excel.SetColumnCombandCellsAndCenterShow(1,i_row_shangxinlieche,1,i_excel_row_lieche2-i_row_shangxinlieche);
		oper_excel.WriteDatatoExcel(1,i_row_shangxinlieche,1,_T("列车进路"));
		//保存一次
		//oper_excel.SaveasXSLFile(strFileSave);
		//int i_excel_row_diaoche2=this->num;
		//	输出上行咽喉调车进路
		iRsCount=strArrayEven.GetCount();
		strBarMsg.Format(_T("上行咽喉调车：%d 进路"),iRsCount);
		acedSetStatusBarProgressMeter(strBarMsg, 0, iRsCount) ;
		for(int i=0;i<strArrayEven.GetCount();i++)
		{
			CStringArray strNextDC;
			...
			if(nHandleStartLine!=0)
			{
				//输出调车进路
				CString strBarMsg2 ;
				int iRsCount=nArrayHandleNext.GetCount();
				strBarMsg2.Format(_T("%s:%d条"), xhj,iRsCount) ;
				acedSetStatusBarProgressMeter(strBarMsg2, 0, iRsCount) ;
				for(int i=0;i<nArrayHandleNext.GetCount();i++)
				{
					long nHandleEndLine=nArrayHandleNext.GetAt(i);
...
// 	//排序输出：
	acutPrintf(_T("\n输出上行咽喉出站兼调车信号机:\n"));
	//CBlkUtility::SortStringArray_CZXHJ(str_name_ArrayXXHJ,_T('S'));
	CBlkUtility::SortStringArray_CZXHJHandle(str_name_ArraySXHJ,nArraySXHJ);
	for(int i=0;i<str_name_ArraySXHJ.GetCount();i++)
	{
		//acutPrintf(_T("%s\t"),str_name_ArraySXHJ.GetAt(i));
	}
	//CString strBarMsg ;
	iRsCount=nArraySXHJ.GetCount();
	strBarMsg.Format(_T("上行咽喉出站兼调车:%d进路"), iRsCount) ;
	acedSetStatusBarProgressMeter(strBarMsg, 0, iRsCount) ;
...
// 		acutPrintf(_T("\n调车始端：%s"),xhj);
	...
	acedRestoreStatusBar();
	int i_row_diaochecurrent4=this->num;
	oper_excel.SetColumnCombandCellsAndCenterShow(1,i_excel_row_lieche2,1,i_row_diaochecurrent4-i_excel_row_lieche2);
	oper_excel.WriteDatatoExcel(1,i_excel_row_lieche2,1,_T("调车进路"));
	oper_excel.SetColumnCombandCellsAndCenterShow(1,i_excel_row_lieche2,2,i_row_diaochecurrent4-i_excel_row_lieche2);
	oper_excel.WriteDatatoExcel(1,i_excel_row_lieche2,2,_T("由"));
	//oper_excel.SaveasXSLFile(strFileSave);
				...
				for(int i=index+1;i<iNum_DC;i++)
				{
					...
					if(this->GetAnotherDuXianDaoCha(nHandle_DXDC,nHandle_DXAno)==TRUE)
					{
						...
						if(IsStart==0&&nHandle_fw!=0&&(IsOntheHorizonline(nHandleLineTdw,nHandle_dwAno,true)==FALSE))
						{
							...
							nHandleDc_DdMin=this->GetDuXianMinDaoCha(nHandle_DXDC,nHandle_DXAno);
							nHandleDc_DdMax=this->GetDuXianMaxDaoCha(nHandle_DXDC,nHandle_DXAno);
							GetDaoChaEntName(nHandleDc_DdMin,str_dc1);
							GetDaoChaEntName(nHandleDc_DdMax,str_dc2);
...
void CZnSxt::GetOnLineDaoChaOrderByXAsc(UINT nLineHandle, CLongArray& nArrayHandleDC)
{
	...
	try
	{
		int i_pos=FindGuiDaoHandleFromVector(nLineHandle);
		if(i_pos!=-1)
		{
			vector<long> l_vec;
			l_vec=vec_GuiDaoInfo[i_pos].m_vecThisDaocha_Asc;
			for(int i=0;i<l_vec.size();i++)
			{
				nArrayHandleDC.Add(l_vec[i]);
			}
		}
// 		CADORecordset cSet(&m_AdoDb) ;
// 				CString strSql ;
...
// 			if(bToward==false)
// 			{
// 				i_vec=vec_GuiDaoInfo[i_pos].m_vecHandleXHJLeft;
// 			}
// 			else
// 			{
// 				i_vec=vec_GuiDaoInfo[i_pos].m_vecHandleXHJRight;
// 			}
// 		}
// 		for(int i=0;i<i_vec.size();i++)
// 		{
// 			nArrayXHJ.Add(i_vec[i]);
// 		}
// // 		if(bToward==false)
// // 		{
// // 			//获取轨道右端信号机
...
// 			{
// 				int toward=-1;
// 				if(GetXhjBlockToward(nArrayHandleXHJTmp[i],toward)==TRUE)
// 				{
// 					if(toward==1)
// 					{
...
void CZnSxt::GetGuiDaoDuanXHJ(bool bforward,UINT nLineHandle, long& ZhiXHJ,long& XiangXHJ)
{
	...
	try
	{
		...
		int i_pos=this->FindGuiDaoHandleFromVector(nLineHandle);
		if(i_pos!=-1)
		{
			if(bforward==false)
			{
				ZhiXHJ=vec_GuiDaoInfo[i_pos].m_nZhixhjLeft;
				XiangXHJ=vec_GuiDaoInfo[i_pos].m_nXiangxhjLeft;
			}
			else
			{
				ZhiXHJ=vec_GuiDaoInfo[i_pos].m_nZhixhjRight;
				XiangXHJ=vec_GuiDaoInfo[i_pos].m_nXiangxhjRight;
			}
		}
	}
	catch(_com_error& e)
	...
	{	
		AfxMessageBox(_T("未知的错误!in GetGuiDaoDuanXHJ"));
	}
}
// 循环查询轨道线连接的信号机及道岔，并按照块的基点x坐标升序排列
...
void CZnSxt::GetOnLineChaOrderByXDesc(UINT nLineHandle, CLongArray& nArrayHandleDC)
{
	...
	try
	{
		...
		{
			vector<long> l_vec;
			l_vec=vec_GuiDaoInfo[i_pos].m_vecThisDaocha_Desc;
			for(int i=0;i<l_vec.size();i++)
			{
				nArrayHandleDC.Add(l_vec[i]);
			}
...
// 需要访问数据库，可以修改为在内存中查询
void CZnSxt::GetXhjEntName( long nHandle,CString &Entname )
{
	try
	{
		CString name;
		...
		//CADORecordset cSet(&m_AdoDb) ;
		//CString strSql ;
		int i_pos=this->FindXhjHandleFromVector(nHandle);
		if(i_pos!=-1)
		{
			//strSql.Format(_T("select * from sxt_ent where sxt_ent_handle=%d"), nHandle) ;
			//m_RecordSet.GetRecordValue(strSql, _T("sxt_ent_name"),name) ;
			name=this->vec_XhjInfo[i_pos].m_strXhjName;
		}
		Entname =name;
		return ;
	}
	...
	{	
		AfxMessageBox(_T("未知的错误!in GetXhjEntName"));
	}	
}
// 需要访问数据库，可以修改为在内存中查询
void CZnSxt::GetDaoChaEntName( long nHandle,CString &Entname )
{
	try
	{
		CString name;
		...
		//CADORecordset cSet(&m_AdoDb) ;
		//CString strSql ;
		int i_pos=this->FindDaoChaHandleFromVector(nHandle);
		if(i_pos!=-1)
		{
			//strSql.Format(_T("select * from sxt_ent where sxt_ent_handle=%d"), nHandle) ;
			//m_RecordSet.GetRecordValue(strSql, _T("sxt_ent_name"),name) ;
			name=this->vec_DaochaInfo[i_pos].m_strDcName;
		}
		Entname =name;
		return ;
	}
...
BOOL CZnSxt::GetXhjBlockPosition_X( long nHandle,double &point_x )
{
	try
	{
		//CString name;
		...
		if(i_pos!=-1)
		{
			point_x=this->vec_XhjInfo[i_pos].m_dXhj_x;
			return TRUE;
		}
		return FALSE;
	}
	...
	{	
		AfxMessageBox(_T("未知的错误!in GetXhjBlockPosition_X"));
		return FALSE;
	}

}
BOOL CZnSxt::GetDaochaBlockPosition_X( long nHandle,double &point_x )
{
	try
	{
		point_x=0;
		...
		if(i_pos!=-1)
		{
			point_x=this->vec_DaochaInfo[i_pos].m_dDcEnt_x;
			return TRUE;
		}
		return FALSE;
	}
	...
	{	
		AfxMessageBox(_T("未知的错误!in :GetDaochaBlockPosition_X"));
		return FALSE;
	}
...
void CZnSxt::FindNextDiaoCheXHJ( long nHandleDC_XHJ,CLongArray& nArrayHandleNext_DC_XHJ )
{
	...
	try
	{
		...
		//获取调车信号机的朝向
		int toward=0;
		if(this->GetXhjBlockToward(nHandleDC_XHJ,toward)==FALSE)
		{
			return;
		}
		long nHandleLineEnd=0;
...
int CZnSxt::FindNextDCByLine( long nHandleLine,long LastDaocha,CStringArray& Gudao,CStringArray& JieJinQuDuan,CLongArray& nArrayHandleNextLine,CStringArray& nArrayDCZhong,bool bfoward )
{
	...
	try
	{
		...
		for(int i=0;i<nArrayHandleNextLineTemp.GetCount();i++)
		{
			...
			long zhixhj=0;
			long xiangxhj=0;
			if(i<0||i>=nArrayHandleThisDaocha.GetCount()||i>nArrayHandleNextLineTemp.GetCount())
			{
				AfxMessageBox(_T("超限 in FindNextDcByLine"));
				return 0;
			}
			...
			CString strDCname;
			//可以优化
			GetDaoChaEntName(HandleDC,strDCname);
			GetGuiDaoDuanXHJ(iToward,nArrayHandleNextLineTemp[i],zhixhj,xiangxhj);
			if(bfoward==true)
			{
				if(GetDaochaBlockPosition_X(HandleDC,pt_DC_x)==TRUE)
				{
					if((GetXhjBlockPosition_X(zhixhj,pt_xhj_x)==TRUE))
					{
						if(pt_xhj_x>pt_DC_x)
						{
							zhixhj=zhixhj;
					...
					if((GetXhjBlockPosition_X(xiangxhj,pt_xhj_x)==TRUE))
					{
						if(pt_xhj_x>pt_DC_x)
						{
							xiangxhj=xiangxhj;
			...
			if(zhixhj!=0)
			{
				...
				CString DCname;
				//可以优化,获取设备名称
				GetXhjEntName(zhixhj,DCname);
				CString EntName;
				//可以优化，获取块名
				GetXhjBlkNameByHandle(zhixhj,EntName);
				if(EntName.Left(6)==_T("XHJ_LF"))
				{
					DCname+=_T("D");
				}
			...
			else
			{
				...
				str32.ForMa
				CString str_xhj_ent;
				GetXhjBlkNameByHandle(xiangxhj,str_xhj_ent);
				//表示为尽头线
				if((xiangxhj!=0)&&(EntName.Left(6)==_T("JYJ_DT")))
				{
					CString DCname;
					GetXhjEntName(xiangxhj,DCname);
					//DCname=_T("向")+DCname;
					//acutPrintf(_T("\n 尽头线信号机：%s"),DCname);
					CString EntName;
					GetXhjBlkNameByHandle(xiangxhj,EntName);
				    if(EntName.Left(6)==_T("XHJ_LF"))
					{
						//向信号机为出站信号机时
						DCname+=_T("D");
...
// 		}
		...
		if(xiangxhj!=0)
		{
			...
			{
				CString blkname;
				GetXhjEntName(xiangxhj,blkname);
				if(EntName.Left(6)==_T("XHJ_LF"))
				{
					//进站信号机无需加_T("L")
		...
		for(int i=0;i<nArrayHandleNextLineTemp.GetCount();i++)
		{
		    ...
		    if(xiangxhj!=0)
			{
				...
				CString blkname;
				GetXhjEntName(nhandleXHJ,blkname);
				long HandleDC=nArrayHandleThisDaocha[i];
				double pt_DC_x=0;
				double pt_xhj_x=0;
				if(bfoward==true)
				{
					if(GetDaochaBlockPosition_X(HandleDC,pt_DC_x)==TRUE)
					{
						if((GetXhjBlockPosition_X(nhandleXHJ,pt_xhj_x)==TRUE))
						{
							if(pt_xhj_x<pt_DC_x)
							{
							   nhandleXHJ=0;
				...
				GetXhjBlkNameByHandle(nhandleXHJ,EntName);
				if(EntName.Left(6)==_T("XHJ_LJ")||EntName.Left(6)==_T("XHJ_LF"))
				{
					CString blkname;
					GetXhjEntName(nhandleXHJ,blkname);
...
int CZnSxt::FindNextDuiXiangDCByLine( long nHandleLine,long LastDaocha,CStringArray& Gudao,CLongArray& nArrayHandleNextLine,CStringArray& nArrayDiDuiDC,bool bfoward )
{
	...
	try
	{
		...
		if(bfoward==true)
		{
			if(GetDaochaBlockPosition_X(LastDaocha,pt_DC_x)==TRUE)
			{
				if((GetXhjBlockPosition_X(zhixhj,pt_xhj_x)==TRUE))
				{
					if(pt_xhj_x>pt_DC_x)
...
// 			m_mysys.GetXdata(nArrayHandleNextLineTemp.GetAt(i),_T("GUIDAONAME"),strGuidaoName2);
// 		}
		if(nArrayHandleThisDaocha.GetCount()!=nArrayHandleNextLineTemp.GetCount())
		{
			AfxMessageBox(_T("两数组个数不相等"));
				//return 0;
		...
		for(int i=0;i<nArrayHandleNextLineTemp.GetCount();i++)
		{
			...
			long zhixhj=0;
			long xiangxhj=0;
			if(i<0||i>=nArrayHandleThisDaocha.GetCount())
			{
				AfxMessageBox(_T("nArrayHandleThisDaocha,超限"));
				return 0;
			}
			long HandleDC=nArrayHandleThisDaocha[i];
			double pt_DC_x=0;
			...
			if(i<0||i>=nArrayHandleNextLineTemp.GetCount()||i>=nArrayHandleThisDaocha.GetCount())
			{
				AfxMessageBox(_T("nArrayHandleNextLineTemp超限，i=%d,cnt=%d"),i,nArrayHandleNextLineTemp.GetCount());
				return 0;
			}
			if(xiangxhj!=0)
			{
				nArrayHandleNextLine.Add(nArrayHandleNextLineTemp.GetAt(i));
				CString DCname;
				GetXhjEntName(xiangxhj,DCname);
				CString EntName;
				GetXhjBlkNameByHandle(xiangxhj,EntName);
			...
			{			
				FindNextDuiXiangDCByLine2(nArrayHandleNextLineTemp.GetAt(i),nArrayHandleThisDaocha.GetAt(i),Gudao,nArrayHandleNextLine,nArrayDiDuiDC,strArrayDaocha,bfoward);
			}
		}
		return 1;
	}
...
/**********************************2015-06-03 am 10:58 by luo********************************************************************/
/**********************************2015-07-10 am 9:48 modified by luo*************************************************************************************************/
/**********************************2016-03-08 pm 18:53 modified by luo**************************************************************************************************/
int CZnSxt::GetNextDCXHJByDCXHJ( const long nHandleDC,CStringArray& Gudao,CStringArray& JieJinQuDuan,CLongArray& nArrayHandleNextLine,CStringArray& strArrayDC )
{
	try
	{
		...
		else
		{
			...
			long LastDaocha=0;
			//m_RecordSet.GetRecordValue(strSql, _T("sxt_ent_conline_left"),nHandleLine) ;
			this->GetXHJ_ConLineLeft(nHandleDC,nHandleLine);
			FindNextDCByLine(nHandleLine,LastDaocha,Gudao,JieJinQuDuan,nArrayHandleNextLine,strArrayDC,false);
			//strSql.Format(_T("select * from sxt_ent where sxt_ent_handle=%d"), nHandleDC) ;
			nHandleLine=0;
			LastDaocha=0;
			//m_RecordSet.GetRecordValue(strSql, _T("sxt_ent_conline_right"),nHandleLine) ;
			this->GetXHJ_ConLineRight(nHandleDC,nHandleLine);
			FindNextDCByLine(nHandleLine,LastDaocha,Gudao,JieJinQuDuan,nArrayHandleNextLine,strArrayDC,true);
			return 0;
		}
		long nHandleLine=0;
...
int CZnSxt::GetXHJ_ConLineLeft(  long nHandleDC,long &nHandleLine)
{
	try
	{
		...
		int i_pos=this->FindXhjHandleFromVector(nHandleDC);
		if(i_pos!=-1)
		{
			nHandleLine=this->vec_XhjInfo[i_pos].m_nHandleXhjLeftLine;
		}
	...
	{	
		AfxMessageBox(_T("未知的错误!in GetXhj_ConLineLeft"));
		return -2;
	}		
}
int CZnSxt::GetXHJ_ConLineRight(  long nHandleDC,long &nHandleLine)
{
	try
	{
		...
		if(i_pos!=-1)
		{
			nHandleLine=this->vec_XhjInfo[i_pos].m_nHandleXhjRightLine;
		}
	...
	{	
		AfxMessageBox(_T("未知的错误!in GetXhj_ConLineRight"));
		return -2;
	}		
}
...
void CZnSxt::GetXhjBlkNameByHandle( const long nHandle,CString &str_ent_name)
{
	try
	{
		//CString strSql ;
		...
		if(i_pos!=-1)
		{
			//strSql.Format(_T("select * from sxt_ent where sxt_ent_handle=%d"), nHandle) ;
			//m_RecordSet.GetRecordValue(strSql, _T("sxt_ent_block"),str_ent_name);
			str_ent_name=this->vec_XhjInfo[i_pos].m_strBlockName;
		}
	...
	{	
		AfxMessageBox(_T("未知的错误!in GetXhjBlkNameByHandle"));
		//return -2;
	}		
...
BOOL CZnSxt::FindNextLineByJYJ( long nHandleLineStart,long &nHandleLineNext,bool bForward )
{
	try
	{
		...
		int i_pos=FindGuiDaoHandleFromVector(nHandleLineStart);
		if(i_pos!=-1)
		{
			if(bForward==true)
			{
				nHandleLineNext=vec_GuiDaoInfo[i_pos].m_nHandleNextLineRight;;

			}
			else
			{
				nHandleLineNext=vec_GuiDaoInfo[i_pos].m_nHandleNextLineLeft;;
			}
			return TRUE;
		}
		return FALSE;
...
// BOOL CZnSxt::FindLineType( long nHandleLineStart,int &iLineType)
// {
// 	try
// 	{
// 		if(nHandleLineStart==0)
...
// 		if(i_pos!=-1)
// 		{
// 			iLineType =vec_GuiDaoInfo[i_pos].m_iLineType;
// 			return TRUE;
// 		}
// 		return FALSE;
// 
...
// 	catch(...)
// 	{	
// 		AfxMessageBox(_T("未知的错误!in FindLineType"));
// 		//return -2;
// 		return FALSE;
// 	}		
// 	return FALSE;
...
//求反位线上的道岔，目前只考虑反位线上的道岔只有一个
BOOL CZnSxt::FindDaocha_Byfwx( long nHandleLine_fw,long &nHandleDC)
{
	try
	{
		if(nHandleLine_fw==0)
		...
		for(int i=0;i<this->vec_DaochaInfo.size();i++)
		{
			if(vec_DaochaInfo[i].m_nHandleLine_fw==nHandleLine_fw)
			{
				nHandleDC=vec_DaochaInfo[i].m_nHandleDC;
				return TRUE;
			}
		}
		return FALSE;
	...
	{	
		AfxMessageBox(_T("未知的错误!in FindDaocha_Byfwx"));
		//return -2;
		return FALSE;
	}		
	return FALSE;
...
BOOL CZnSxt::FindCXGuiDao( const long nHandleCXJYJ,long & nHandleLineConLeft,long& nHandleLineCXLeft,long& nHandleLineConRight,long& nHandleLineCXRight ,CString& str_CX_GdName_left,CString &str_CX_GdName_Right)
{
	try
	{
	   ...
	   // acutPrintf(_T("\n找警冲标"));
		...
		{
			CString str_DC;
			GetDaoChaEntName(nArrayHandleLeftDC.GetAt(i),str_DC);
			if(i!=iCount_left-1)
			{
		...
		{
			CString str_DC;
			GetDaoChaEntName(nArrayHandleRightDC.GetAt(i),str_DC);
			if(i!=iCount_right-1)
			{
...
// 	sign_point1.y+=17;
	...
	CString strFilePath ;
	strFilePath.Format(_T("%s\\support\\sign\\SIGN_6.dwg"), g_strBasePath) ;
